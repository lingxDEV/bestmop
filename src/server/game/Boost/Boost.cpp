...
void CharacterBooster::_LearnSpells(SQLTransaction& trans, uint8 const& raceId, uint8 const& classId) const
{
    ...
    else if (raceId == RACE_PANDAREN_ALLI)
...
uint8 CharacterBooster::_GetRace() const
{
    ...
    if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
    {
        ...
        if (charRace == RACE_PANDAREN_NEUTRAL)
            charRace = m_charBoostInfo.allianceFaction ? RACE_PANDAREN_ALLI : RACE_PANDAREN_HORDE;
    }
...
void CharacterBooster::_HandleCharacterBoost() const
{
    ...
    if (ChrSpecializationsEntry const* specEntry = sChrSpecializationsStore.LookupEntry(m_charBoostInfo.specialization))
