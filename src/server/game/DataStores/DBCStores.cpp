...
std::map<uint32, std::set<ResearchProjectEntry const*>> sResearchProjectMap;
...
std::map<uint32 /*spellId*/, SpellItemEnchantmentEntry const*> sSpellEnchantScalingData;
...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sAchievementCriteriaStore,    dbcPath, "Achievement_Criteria.dbc", &CustomAchievementCriteriafmt, &CustomAchievementCriteriaIndex);                                      // 17399
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sItemSpecStore,               dbcPath, "ItemSpec.dbc");                                                     // 17399
	...
	for (uint32 i = 0; i < sResearchProjectStore.GetNumRows(); ++i)
	{
		...
		sResearchProjectMap[rp->branchId].insert(rp);
	}

	ASSERT(!sResearchProjectMap.empty());
    ...
    {
        if (SpellItemEnchantmentEntry const* ench = sSpellItemEnchantmentStore.LookupEntry(i))
        {
            if (ench->ScalingClass == 0)
                continue;
            ...
            for (uint8 j = 0; j < 3; ++j)
            {
                ...
                if (!sSpellStore.LookupEntry(ench->spellid[j]))
                    continue;

                sSpellEnchantScalingData[ench->spellid[j]] = ench;
            }
        }
    }
    ...
    // Load GameObject Transports
    LoadDBC(availableDbcLocales, bad_dbc_files, sTransportAnimationStore,     dbcPath, "TransportAnimation.dbc");                                           // 17399
    ...
    LoadDBC(availableDbcLocales, bad_dbc_files, sUnitPowerBarStore,           dbcPath, "UnitPowerBar.dbc");                                                 // 17399
...
float GetBattlePetDamageMod(int32 attackerType, int32 victimType)
{
	uint32 index = (10 * victimType) + attackerType;
	GtBattlePetTypeDamageModEntry const* entry = sGtBattlePetTypeDamageModStore.LookupEntry(index);
	if (!entry)
		return 1.0f;
...
bool IsPetStrongAgainst(int32 attackerFamily, int32 victimFamily)
{
	uint32 index = (10 * victimFamily) + attackerFamily;
	GtBattlePetTypeDamageModEntry const* entry = sGtBattlePetTypeDamageModStore.LookupEntry(index);
	if (!entry)
		return false;

	return entry->modifier > 1.0f;
}

bool IsPetWeakAgainst(int32 attackerFamily, int32 victimFamily)
{
	uint32 index = (10 * victimFamily) + attackerFamily;
	GtBattlePetTypeDamageModEntry const* entry = sGtBattlePetTypeDamageModStore.LookupEntry(index);
	if (!entry)
