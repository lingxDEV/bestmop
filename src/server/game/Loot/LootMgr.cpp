...
class LootTemplate::LootGroup                                                       // A set of loot definitions for items (refs are not allowed)
{
    public:
        ...
        void Process(Loot& loot, uint16 lootMode, Player* looter = NULL) const;     // Rolls an item from the group (if any) and adds the item to the loot
...
LootItem::LootItem(LootStoreItem const& li, Loot* loot, bool extraItems)
{
    currentLoot = loot;
    itemid      = li.itemid;
    type        = li.type;
    ...
    //https://us.battle.net/forums/en/wow/topic/8178378102?page=2 and https://wow.gamepedia.com/Bountiful_Bags
    if (extraItems)
        count = urand(3, 5);
...
void Loot::AddItem(LootStoreItem const & item, Player* looter, bool extraItems)
{
    if (item.needs_quest)                                   // Quest drop
    {
        if (quest_items.size() < MAX_NR_QUEST_ITEMS)
            quest_items.push_back(LootItem(item, this, extraItems));
    }
    else if (items.size() < MAX_NR_LOOT_ITEMS)              // Non-quest drop
    {
        items.push_back(LootItem(item, this, extraItems));
        ...
        proto->SubClass == ITEM_SUBCLASS_ENCHANTING))
    {
        if (looter)
        {
            float amount = looter->GetTotalAuraModifier(SPELL_AURA_MOD_GATHERING_ITEMS_GAINED_PERCENT);

            if (roll_chance_i(amount))
                AddItem(item, looter, true);
        }
    }
}
...
void LootTemplate::LootGroup::Process(Loot& loot, uint16 lootMode, Player* looter) const
{
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId, Player* looter) const
{
    if (groupId)                                            // Group reference uses own processing of the group
    {
        ...
        Groups[groupId - 1].Process(loot, lootMode, looter);
        return;
    }
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        if (i->mincountOrRef < 0 && i->type == LOOT_ITEM_TYPE_ITEM)                             // References processing
        {
            ...
            for (uint32 loop = 0; loop < maxcount; ++loop)    // Ref multiplicator
                Referenced->Process(loot, rate, lootMode, i->group, looter);
        }
        else                                                  // Plain entries (not a reference, not grouped)
            loot.AddItem(*i, looter);                                   // Chance is already checked, just add
    }

    // Now processing groups
    for (LootGroups::const_iterator i = Groups.begin(); i != Groups.end(); ++i)
        i->Process(loot, lootMode, looter);
}
...
void BonusLootProcessor::ProcessItemLootFor(Player* player)
{
    ...
    else
    {
        ...
        {
            player->AddItem(l_Items[selectItem], 1);
            player->SendDisplayToast(l_Items[selectItem], 1, TOAST_TYPE_NEW_ITEM, false, false);
...
void CreatureBonusLootProcessor::GetPlayersWithBonus(std::list<Player*>& players, float range)
{
    ...
    WoWSource::PlayerListSearcher<PlayerWithCurrencyCheck> searcher(m_Creature, players, checker);
