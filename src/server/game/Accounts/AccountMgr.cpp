...
namespace AccountMgr
{
    ...
    AccountOpResult ChangeWoDPassword(uint32 accountId, std::string newPassword)
    {
        std::string username;
        ...
        std::string passwordVerifier = SRP6aCalculatePasswordVerifier(username + "@rog.snet", newPassword, salt);

        // everything else requires an uppercase email
        normalizeString(username, true);

        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_WOD_PASSWORD);

        stmt->setString(0, CalculateShaPassHash(username + "@ROG.SNET", newPassword));
        stmt->setString(1, passwordVerifier);
        stmt->setString(2, salt);
        stmt->setString(3, username + "@ROG.SNET");
