...
WorldSession::WorldSession(uint32 id, WorldSocket* sock, AccountTypes sec, bool ispremium, uint8 expansion, uint8 viplevel, time_t mute_time, LocaleConstant locale, uint32 recruiter, bool isARecruiter, bool hasBoost) :
m_muteTime(mute_time), m_timeOutTime(0), _player(NULL), m_Socket(sock),
_security(sec), _ispremium(ispremium), _accountId(id), m_expansion(expansion), 
m_charBooster(new CharacterBooster(this)), 
m_viplevel(viplevel), _logoutTime(0),
...
m_hasBoost(hasBoost),timeLastWhoCommand(0),
...
timeLastChannelModerCommand(0), timeLastChannelOwnerCommand(0), m_TimeLastTicketOnlineList(0),
...
#define MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE 250
    ...
    if (updater.ProcessLogout())
    {
        ...
        {
			// Avoiding Instant Logouts with ALT+F4
