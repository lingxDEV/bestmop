...
WorldSocket::WorldSocket (void): WorldHandler(),
    ...
    m_WorldHeader(sizeof(WorldClientPktHeader)), m_OutBuffer(0), m_OutBufferSize(65536),
    m_OutActive(false), m_Seed(static_cast<uint32> (rand32())), m_zstream()
{
...
int WorldSocket::handle_input_header (void)
{
    ...
    if (m_Crypt.IsInitialized())
    {
        ...
        ACE_NEW_RETURN(m_RecvWPct, WorldPacket((Opcodes)opcodeNumber, header.size), -1);
...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
	...
	// Update last apttemt ip
	PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_LAST_ATTEMPT_IP);
	stmt->setString(0, GetRemoteAddress().c_str());
	stmt->setUInt32(1, fields[0].GetUInt32());
    ...
    */
	...
	ACE_NEW_RETURN(m_Session, WorldSession(id, this, AccountTypes(security), isPremium, expansion, m_viplevel, mutetime, locale, recruiter, isRecruiter, hasBoost), -1);
