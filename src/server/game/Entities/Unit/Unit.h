...
enum filterTargetsMask
{
    EXCLUDE_ALLIES            = 0x00000001,
    EXCLUDE_ENEMIES           = 0x00000002,
    ONLY_LOS                  = 0x00000004,
    ONLY_PLAYERS              = 0x00000008,
    ONLY_RAID_MEMBERS         = 0x00000010,
    ONLY_ARC                  = 0x00000020,
    HAS_SPECIFIC_AURA         = 0x00000040,
    HAS_NOT_SPECIFIC_AURA     = 0x00000080,
    INCLUDE_NOT_VALID_ASSIST  = 0x00000100,
    INCLUDE_NOT_VALID_ATTACK  = 0x00000200,
    INCLUDE_CRITTERS          = 0x00000400,
    INCLUDE_TOTEMS            = 0x00000800
};
...
enum ListOrder
{
    RandomOrderPred,
    PowerPctOrderPred,
    UnitDistanceCompareOrderPred,
    HealthPctOrderPred,
    DistanceCompareOrderPred,
    DistanceOrderPred,
};
...
struct HealDamageLog
{
    HealDamageLog()
        : m_value(0), m_time(0), m_type(DAMAGE_DONE_LOG)
    {
    }

    HealDamageLog(uint32 value, uint32 time, HealDamageLogType type)
        : m_value(value), m_time(time), m_type(type)
    {
    }

    uint32 m_value;
    uint32 m_time;
    HealDamageLogType m_type;
};
...
class Unit : public WorldObject
{
    public:
        ...
        bool CanIgnoreMinRange(Unit const* target, SpellInfo const* spellInfo) const;
        ...
        void FilterTargets(std::list<Unit*> &targetList, float range, uint32 mask = EXCLUDE_ENEMIES, uint32 maxTargets = NULL, Unit* exclude = NULL, ListOrder listOrder = RandomOrderPred, float arcAngle = 180.f, int32 aura = NULL, uint64 auraOwnerGUID = 0, bool ascendingOrder = true, ArcPosition arcPos = Front) const;
		...
		{ 
			uint32 maxHp = GetUInt32Value(UNIT_FIELD_MAXHEALTH);
			uint32 hp = GetUInt32Value(UNIT_FIELD_HEALTH);
			return GetUInt32Value(UNIT_FIELD_MAXHEALTH) ? 100.0f * (((float)hp) / ((float)maxHp)) : 0.0f;
		...
		int32 GetCurrentPower()const{ return m_realPower; }
		...
		int32 CapForPVP(Unit* victim, uint32 value, bool damage, SpellInfo const* spellInfo = NULL);
        ...
        void CastWithDelay(uint32 delay, Unit* victim, uint32 spellid, bool triggered = false, bool repeat = false, Item* castItem = NULL, AuraEffectPtr triggeredByAura = NULLAURA_EFFECT, uint64 originalCaster = 0);
        void CastWithDelayPos(float x, float y, float z, uint32 delay, uint32 spellid, bool triggered, bool repeat);
		AuraPtr ToggleAura(uint32 spellId, Unit* target);
        AuraPtr AddAura(uint32 spellId, Unit* target, uint64 casterGuid = 0);
        AuraPtr AddAura(SpellInfo const* spellInfo, uint32 effMask, Unit* target, uint64 casterGuid = 0);
        void SetAuraStack(uint32 spellId, Unit* target, uint32 stack);
        void SendPlaySpellVisualKit(uint32 id, uint32 unkParam);
        void SendPlaySpellVisual(uint32 id, Unit* target, float travelSpeed, bool thisAsPos = false, bool speedAsTime = false);
        ...
        */
        ...
        AuraPtr _TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint32 effMask, Unit* caster, int32* baseAmount = NULL, Item* castItem = NULL, uint64 casterGUID = 0, int32 castItemLevel = -1);
        ...
        void RemoveSnareAuras() { RemoveAurasWithMechanic(1 << MECHANIC_SNARE); }
        void RemoveRootAuras() { RemoveAurasWithMechanic(1 << MECHANIC_ROOT); }
        void RemoveMovementImpairingAuras();
        void RemoveAurasBreakableByDamage();
        void RemoveDKPresences();
        ...
        int32 GetTotalAuraModifierByMiscValue(AuraType auratype, int32 misc_value, int32 misc_valueB = 0) const;
        ...
        bool SetAndHandleModifierValue(UnitMods unitMod, UnitModifierType modifierType, float value);
        ...
        void SetDisplayId(uint32 modelId, bool removeCloneAura = true);
        ...
        int32 CalculateSpellDamage(Unit const* target, SpellInfo const* spellProto, uint8 effect_index, int32 const* basePoints = NULL, int32 itemLevel = -1) const;
        ...
        int32 GetHighestExclusiveSameEffectSpellGroupValue(constAuraEffectPtr aurEff, AuraType auraType, int32 miscValue = NULL, int32 miscValueB = NULL) const;
        ...
        void AddHealDamageLog(HealDamageLog const& log) { m_healDamage.push_back(log); }
		...
		BattlePet::Battle* GetPetBattle();

		void SetPetBattleId(uint64 id);

        template<typename F>
        void ForEachDynObject(F p_Func)
        {
            ForEach(m_dynObj, std::move(p_Func));
        }
    ...
    protected:
        ...
        void ForEach(L& p_List, F p_Func)
        {
            for (auto l_Object : p_List)
            {
                if (!p_Func(l_Object))
                    break;
            }
        ...
        Unit* m_attacking;
		// Esto es para la funcion de CAP pues el hunter pirde el target cunado apliza el poder de hacerse el muerto
		uint32 m_hunterTargetlive;
		uint32 m_huntertargetgear;
		TypeID m_hunterTargetType;
		uint8 m_hunterTargetLevel;
		bool m_hunterGuardian;
		...
		std::vector<AuraPtr> m_removedAuras;
...
namespace WoWSource
{
    ...
    // Binary predicate for sorting AreaTriggers based on value of duration
    ...
    class DynamicObjectsDurationPctOrderPred
    {
    public:
        DynamicObjectsDurationPctOrderPred(bool ascending = true) : m_ascending(ascending) {}
