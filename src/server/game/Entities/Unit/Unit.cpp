...
uint64 PetEntryList[46] = {
	// Paladin
	46499, // Temp Summon. - Holy.
	46490, // Temp Summon. - Protection.
	46506, // Temp Summon. - Retribution.

	// Warlock
	// -- Normal Pets.
	416,//ENTRY_IMP  
	1860,//ENTRY_VOIDWALKER 
	1863,//ENTRY_SUCCUBUS
	417,//ENTRY_FELHUNTER
	17252,//ENTRY_FELGUARD 
	89,//ENTRY_INFERNAL  Temp Summon.
	11859,//ENTRY_DOOMGUARD  Temp Summon.

	// -- With Grimoire of Supremacy.
	58959,//ENTRY_FEL_IMP  
	58960,//ENTRY_VOIDLORD  
	58963,//ENTRY_SHIVARRA  
	58964,//ENTRY_OBSERVER  
	58965,//ENTRY_WRATHGUARD  
	58997,//ENTRY_ABYSSAL   // Temp Summon.
	59000,//ENTRY_TERRORGUARD   // Temp Summon.

	55659,//ENTRY_WILD_IMP =  // Temp Summon.
	50675,//ENTRY_EBON_IMP =  // Temp Summon.

	// Mage
	510,//ENTRY_WATER_ELEMENTAL = 
	31216,//ENTRY_MIRROR_IMAGE =  // Temp Summon.

	// Priest
	19668,//ENTRY_SHADOWFIEND   Temp Summon.
	59190,//ENTRY_PSYFIEND Temp Summon.
	62982,//ENTRY_MINDBENDER  Temp Summon.
	67236,//ENTRY_MINDBENDER_SHA Temp Summon.
	46954,//ENTRY_SHADOWY_APPAR  Temp Summon.
	61966,//ENTRY_SHADOWY_APPAR_2 Temp Summon.

	// Hunter
	19921,//ENTRY_VIPER Temp Summon.
	19833,//ENTRY_VENOMOUS_SNAKE Temp Summon.

	61994,//ENTRY_CROW Temp Summon.

	// Druid
	54985,//ENTRY_TREANT_GUARDIAN  
	54984,//ENTRY_TREANT_FERAL  
	54983,//ENTRY_TREANT_RESTO  
	1964,//ENTRY_TREANT_BALANCE  

	// Shaman
	15352,//ENTRY_EARTH_ELEMENTAL 
	15438,//ENTRY_FIRE_ELEMENTAL 
	29264,//ENTRY_SPIRIT_WOLF 
	58488,//ENTRY_SPIRIT_WOLF_S 
	59764,//ENTRY_HEALING_T_TOTEM
	61056,//ENTRY_EARTH_ELEM_TOTEM 
	61029,//ENTRY_FIRE_ELEM_TOTEM 

	// Death Knight
	26125,//ENTRY_GHOUL 
	28017,//ENTRY_BLOODWORM
	27829,//ENTRY_GARGOYLE 

	24207,//ENTRY_AOD_GHOUL 

	// Monk
	63508//ENTRY_XUEN_WHITE_TIGER   // Temp Summon.
};
...
void Unit::Update(uint32 p_time)
{
    ...
    for (HealDamageLogList::iterator itr = m_healDamage.begin(); itr != m_healDamage.end();)
    {
        ...
        if ((getMSTime() - ptr.m_time) > 60 * IN_MILLISECONDS)
        {
...
{
    HealDamageLog dmgTaken(damage, getMSTime(), DAMAGE_TAKE_LOG);
    victim->AddHealDamageLog(dmgTaken);

    HealDamageLog dmgDone(damage, getMSTime(), DAMAGE_DONE_LOG);
    AddHealDamageLog(dmgDone);
    ...
    // Custom MoP Script - Backlash
    if (plr && this && plr->getClass() == CLASS_WARLOCK && plr->GetSpecializationId(plr->GetActiveSpec()) == SPEC_WARLOCK_DESTRUCTION)
    {
        if (damageSchoolMask == SPELL_SCHOOL_MASK_NORMAL)
        {
            if (!plr->HasSpellCooldown(108563) && roll_chance_i(25))
            {
                plr->AddAura(34936, plr);
                plr->AddSpellCooldown(108563, 0, time(NULL) + 8);
            }
        }
    }

    // Howl of Terror CD Reduction
    if (plr && this && plr->getClass() == CLASS_WARLOCK)
    {
    ...
    if (this && victim && damagetype == DIRECT_DAMAGE && this->HasAura(33757) && this->HasAura(118472) && this->HasAura(324) && this->HasAura(117012))
    {
        if (roll_chance_i(45))
    ...
    if (this && victim && damagetype == SPELL_DIRECT_DAMAGE && this->HasAura(31224))
    {
        damage = 0;
    }
    */
    ...
    if (victim->HasAura(115610) && !victim->HasAura(115611))
		victim->CastSpell(victim, 115611, true);
    ...
    if (owner && ((GetTypeId() == TYPEID_UNIT && GetEntry() == 15438 && !spellProto) || (isTotem() && GetEntry() == 2523)) && owner->HasAura(77657) && owner->GetSpecializationId(owner->GetActiveSpec()) == SPEC_SHAMAN_ENHANCEMENT)
        owner->CastSpell(owner, 77661, true);
    ...
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->getClass() == CLASS_MONK && spellProto
        && spellProto->Id != 124098 && spellProto->Id != 107270 && spellProto->Id != 132467
        && spellProto->Id != 130651 && spellProto->Id != 117993 && HasAura(115070)) // Don't triggered by Zen Sphere, Spinning Crane Kick, Chi Wave, Chi Burst and Chi Torpedo
    {
        int32 bp = damage / 2;
...
bool Unit::_IsInCC()
{
	...
	{
		//if (HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_FLEEING) || HasAuraType(SPELL_AURA_MOD_FEAR) || HasAuraType(SPELL_AURA_MOD_FEAR_2))
		return false;
	}
#endif
}
...
void Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem, constAuraEffectPtr triggeredByAura, uint64 originalCaster, float periodicDamageModifier /* = 0.0f*/)
{
	...
	if (spellInfo && spellInfo->GetTriggeredByHavoc())
		spell->GetSpellInfo()->SetTriggeredByHavoc(spellInfo->GetTriggeredByHavoc());
...
void Unit::CastWithDelay(uint32 delay, Unit* victim, uint32 spellid, bool triggered, bool repeat, Item* castItem, AuraEffectPtr triggeredByAura, uint64 originalCaster)
{
	...
	public:
        CastDelayEvent(Unit* _me, Unit* _victim, uint32 const& _spellId, bool const& _triggered, bool const& _repeat, uint32 const& _delay,
            Item* _castItem, AuraEffectPtr const& _triggeredByAura, uint64 const& _originalCaster) :
            me(_me), victim(_victim), spellId(_spellId), triggered(_triggered), repeat(_repeat), delay(_delay),
            castItem(_castItem), triggeredByAura(_triggeredByAura), originalCaster(_originalCaster) { }

		bool Execute(uint64 /*execTime*/, uint32 /*diff*/)
		{
            me->CastSpell(victim, spellId, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE, castItem, triggeredByAura, originalCaster);
			if (repeat)
                me->CastWithDelay(delay, victim, spellId, triggered, repeat, castItem, triggeredByAura, originalCaster);
			return true;
		}
	...
	private:
		...
		uint32 const delay;
        uint64 const originalCaster;
        AuraEffectPtr const triggeredByAura;
        Item* castItem;
		bool const triggered;
		bool const repeat;
	};

    m_Events.AddEvent(new CastDelayEvent(this, victim, spellid, triggered, repeat, delay, castItem, triggeredByAura, originalCaster), m_Events.CalculateTime(delay));
}

void Unit::CastWithDelayPos(float x, float y, float z, uint32 delay, uint32 spellid, bool triggered, bool repeat)
{
    class CastDelayEvent : public BasicEvent
    {
    public:
        CastDelayEvent(float _x, float _y, float _z, Unit* _me, uint32 const& _spellId, bool const& _triggered, bool const& _repeat, uint32 const& _delay) :
            x(_x), y(_y), z(_z), me(_me), spellId(_spellId), triggered(_triggered), repeat(_repeat), delay(_delay) { }

        bool Execute(uint64 /*execTime*/, uint32 /*diff*/)
        {
            me->CastSpell(x, y, z, spellId, triggered);
            if (repeat)
                me->CastWithDelayPos(x, y, z, delay, spellId, triggered, repeat);
            return true;
        }
    ...
    m_Events.AddEvent(new CastDelayEvent(x, y, z, this, spellid, triggered, repeat, delay), m_Events.CalculateTime(delay));
}
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    // Soul link
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->getClass() == CLASS_WARLOCK && victim->HasAura(108446))
    {
        bool isSoulLinkDmg = false;
        if (spellInfo && spellInfo->Id == 108446)
            isSoulLinkDmg = true;
        ...
        if (!isSoulLinkDmg)
        {
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(108446))
            {
                ...
                if (splitDamage)
                {
                    if (victim->GetTypeId() == TYPEID_PLAYER)
                    {
                        if (Pet* pet = victim->ToPlayer()->GetPet()) // If the player is attacked the pet takes 20% of dmg.
                        {
                            if (pet->GetHealth() < splitDamage)
                                splitDamage = pet->GetHealth();

                            victim->SendSpellNonMeleeDamageLog(pet, 108446, splitDamage, SPELL_SCHOOL_MASK_SHADOW, 0, 0, false, 0, false);
                            victim->DealDamage(pet, splitDamage, NULL, SPELL_DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, spellInfo, true);
                        }
                    }
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
            ...
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(108446))
            {
                ...
                if (splitDamage)
                {
                    if (Pet* pet = victim->ToPlayer()->GetPet()) // If the player is attacked the pet takes 20% of dmg.
                    {
                        ...
                        victim->SendSpellNonMeleeDamageLog(pet, 108446, splitDamage, SPELL_SCHOOL_MASK_NORMAL, 0, 0, true, 0, false);
                        victim->DealDamage(pet, splitDamage, NULL, SPELL_DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, spellInfo, true);
                    }

                    damageInfo->damage -= splitDamage;
                }
            }
        }
    }
    ...
    if (damageInfo->attackType == DIRECT_DAMAGE)
    {
        ...
        if (victim->ToPlayer() && victim->getClass() == CLASS_MONK)
        {
            if (victim->HasAura(115176) || victim->HasAura(131523))
            {
                victim->RemoveAurasDueToSpell(115176);
                victim->RemoveAurasDueToSpell(131523);
            }
        }
    }
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
	...
	if (spellInfo)
		sLog->OutSpecialLog("Spell %s: CalcArmorReducedDamage(): %i - %f = %i", spellInfo->SpellName, damage, tmpvalue * 100.f, newdamage);
...
void Unit::AttackerStateUpdate (Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
		...
		damageInfo.damage = CapForPVP(damageInfo.target, damageInfo.damage, true);
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    {
        if (attType != RANGED_ATTACK) // Ranged attacks can't be parried.
        {
            ...
            if (victim->GetTypeId() == TYPEID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY))
            {
                ...
                if (tmp2 > 0 && (tmp2 -= skillBonus) > 0 && roll < (sum += tmp2)) // check if unit _can_ parry
                {
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (attType == RANGED_ATTACK)
    {
         ...
         *
         *  Some hunter spells Like Steady Shoot and Aimed Shot do physical damage and must be evaded 
         *  by Rogue (is this have spell Evasion Activated). So is needed some check to allow this with 
         *  ranged spells.
         */
        if ((victim->GetTypeId() == TYPEID_PLAYER) && (spell->SchoolMask == SPELL_SCHOOL_MASK_NORMAL) && (spell->Dispel == DISPEL_NONE))
            canDodge = true;
    }
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spell, bool CanReflect)
{
    ...
    if (CanReflect)
    {
        ...
        {
            // From 5.3 patch Pets and Guardians spells don't remove reflect effect from victim.
            if (!this->isPet() && !this->isGuardian())
...
AuraPtr Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint32 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, uint64 casterGUID /*= 0*/, int32 castItemLevel /*= -1*/)
{
    ...
    if (!newAura->IsMultiSlotAura())
    {
        ...
        if (foundAura != NULLAURA)
        {
            ...
            int32 oldCooldown = 0;
            // Pandemic add old cooldown when reapply or reupdate
            if (foundAura->HasEffectType(SPELL_AURA_PERIODIC_DAMAGE) && foundAura->GetCaster() && foundAura->GetCaster()->HasAura(131973))
                oldCooldown = foundAura->GetMaxDuration() - foundAura->GetDuration();
            ...
            if (oldCooldown != 0)
            {
                int32 newCooldown = foundAura->GetMaxDuration() + oldCooldown > foundAura->GetMaxDuration() + foundAura->GetMaxDuration() * 0.5 ? foundAura->GetMaxDuration() + foundAura->GetMaxDuration() * 0.5 : foundAura->GetMaxDuration() + oldCooldown;
                foundAura->SetDuration(newCooldown);
            }
...
void Unit::_RemoveNoStackAurasDueToAura(AuraPtr aura)
{
    ...
    for (AuraApplicationMap::iterator i = m_appliedAuras.begin(); i != m_appliedAuras.end(); ++i)
    {
        ...
        // Hack fix Symbiosys
        if ((i->second->GetBase()->GetId() == 110478 ||
            i->second->GetBase()->GetId() == 110479 ||
            i->second->GetBase()->GetId() == 110482 ||
            i->second->GetBase()->GetId() == 110483 ||
            i->second->GetBase()->GetId() == 110484 ||
            i->second->GetBase()->GetId() == 110485 ||
            i->second->GetBase()->GetId() == 110486 ||
            i->second->GetBase()->GetId() == 110488 ||
            i->second->GetBase()->GetId() == 110490 ||
            i->second->GetBase()->GetId() == 110491))
            continue;
...
{	
	for (AuraEffectList::iterator iter = m_modAuras[auraType].begin(); iter != m_modAuras[auraType].end();)
	{
		try
		{
			AuraPtr aura = (*iter)->GetBase();// Esta dando problema en esta linea, insicutiblemente el iterador debe esta en null
			if (!aura)
			{
				++iter;
				continue;
		...
		{
			sLog->outInfo(LOG_FILTER_WORLDSERVER, "Unknwon exception catch at void Unit::RemoveAurasByType");
			break;
		}
	}
}
...
bool Unit::HasAuraWithMechanic(uint32 mechanicMask)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end(); ++iter)
    {
        ...
        AuraPtr auracheck = iter->second->GetBase();
        if (!auracheck)
            continue;
...
int32 Unit::GetTotalAuraModifierByMiscValue(AuraType auratype, int32 misc_value, int32 misc_valueB) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((!misc_value || (*i)->GetMiscValue() == misc_value) && (!misc_valueB || (*i)->GetMiscValueB() == misc_valueB))
            if (!sSpellMgr->AddSameEffectStackRuleSpellGroups((*i)->GetSpellInfo(), (*i)->GetAmount(), SameEffectSpellGroup))
                modifier += (*i)->GetAmount();
    }
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (dummySpell->Id)
            {
                ...
                case 64411:
                {
                    ...
                    if (AuraEffectPtr aurEff = victim->GetAuraEffect(64413, EFFECT_0))
        ...
        case SPELLFAMILY_WARLOCK:
        {
            switch (dummySpell->Id)
            {
                ...
                {
                    if (procSpell && (procSpell->Id == 27285 || procSpell->Id == 27243))
                        return false;

                    if (!triggeredByAura->GetBase()->GetEffect(0))
                        return false;

                    int32 explodeDamage = triggeredByAura->GetBase()->GetEffect(EFFECT_2)->GetAmount();

                    // if damage is more than need or target die from damage deal finish spell
                    if ((explodeDamage <= int32(damage) && triggeredByAura->GetCasterGUID() == victim->GetGUID()) || GetHealth() <= damage)
                    {
                    ...
                    // Damage counting
                    triggeredByAura->GetBase()->GetEffect(EFFECT_2)->ChangeAmount(explodeDamage - damage);
                    return true;
                }
                case 114790:// Soulburn : Seed of Corruption
                {
                    if (procSpell && (procSpell->Id == 87385 || procSpell->Id == 114790))
                        return false;
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if (dummySpell->Id == 47515)
            {
                ...
                if (AuraPtr divineAegis = target->GetAura(47753))
            ...
            switch (dummySpell->Id)
            {
                ...
                case 15286:
                {
                    ...
                    GetPartyMembers(members);

                    uint32 amountTargets = 0;
                    for (auto member : members)
                    {
                        if (member && IsInRange(member, 0.0f, 40.0f, false))
                        {
                    ...
                    SpellInfo const* ProcSpell = sSpellMgr->GetSpellInfo(15290);

                    if (ProcSpell == nullptr)
                        return false;

                    for (auto Target : membersInRange)
                    {
                        if (Target != nullptr)
                            HealBySpell(Target, ProcSpell, bp, false);
                    }
                    return true;                                // no hidden cooldown
                }
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (dummySpell->SpellIconID == 2860)
            {
                ...
                if (AuraEffectPtr livingSeedAura = victim->GetAuraEffect(triggered_spell_id, EFFECT_0, GetGUID()))
        ...
        case SPELLFAMILY_PALADIN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 144580:
                {
                    ...
                    auto nextAmount = triggeredByAura->GetAmount() + int32(damage * 0.75f);
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 16196: // Resurgence
                {
                    ...
                    if (bp0)
                        CastCustomSpell(this, 101033, &bp0, NULL, NULL, true);
            ...
            switch (dummySpell->Id)
            {
                ...
                case 137639:// Storm, Earth and Fire
                {
                    ...
                    if (firstSpirit && (firstSpirit->getVictim() || getVictim()))
                        if (firstSpirit->getVictim() != victim)
                    ...
                    if (secondSpirit && (secondSpirit->getVictim() || getVictim()))
                        if (secondSpirit->getVictim() != victim)
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            switch (dummySpell->Id)
            {
                ...
                case 74434:
                {
                    if (procSpell->Id == 6353 && HasAura(105787))
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffectPtr triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 20165:
        {
            ...
            target = this;
            basepoints0 = GetTotalAttackPowerValue(BASE_ATTACK) * 0.15 + ToPlayer()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY) * 0.15;
            ...
            if (HasAura(119477))
            {
                ...
                {
                    if (!lowhealtarget)
                        lowhealtarget = object;
                    if (object->GetHealth() < lowhealtarget->GetHealth())
                        lowhealtarget = object;
                }
                if (lowhealtarget)
                {
                    target = lowhealtarget;
                    basepoints0 *= 0.3;
                }
            }
            break;
        ...
        case 126154:
        {
            if (damage >= victim->GetMaxHealth() * 0.35)
        ...
        case 85416: // Grand Crusader gives holy power.
        {
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth, SpellInfo const* spellProto)
{
    ...
    HealDamageLog healTaken(addhealth, getMSTime(), HEAL_TAKE_LOG);
    AddHealDamageLog(healTaken);

    HealDamageLog healDone (addhealth, getMSTime(), HEAL_DONE_LOG);
    AddHealDamageLog(healDone);
    ...
    else if (unit && unit->GetTypeId() == TYPEID_PLAYER && unit->getLevel() >= 80 && addhealth > 0 && spellProto && spellProto->Id != 124041 && unit->HasAura(117907))
    {
        ...
        if (roll_chance_f(Mastery * scaling))
        {
            ...
            WoWSource::AnyFriendlyUnitInObjectRangeCheck u_check(unit, unit, 40.0f);
...
bool Unit::IsPlayerPet(uint64 guid){
	for (int i = 0; i < 46; i++){
		if (PetEntryList[i] == guid)
			return true;
	}
	return false;
}

int32 Unit::CapForPVP(Unit* victim, uint32 value, bool damage, SpellInfo const* spellInfo)
{
	// The limit should not work on dungeon or raids
	if (victim)
	{
	...
	// Check victim
	if (!victim && !m_hunterTargetType && !this->HasAura(5384)) // Feign Death
		return value;
	else if (((!this->IsPetGuardianStuff() || (!this->IsControlledByPlayer() && !this->ToPlayer())) || (!this->ToPlayer())) &&
		(victim && ((!victim->IsPetGuardianStuff() || (!victim->IsControlledByPlayer() && !victim->ToPlayer())) && !victim->ToPlayer())) &&
		(victim && IsPlayerPet(this->GetGUID()) && !victim->ToPlayer()))
		return value;
	// Spells that heals 100% player's HP, like Lay on Hands [633]
	if (spellInfo)
	if (spellInfo->HasEffect(SPELL_EFFECT_HEAL_MAX_HEALTH))
		return value;

	// A partir de aqui estamos obteniendo datos basicos para los calculos

	if (this->getClass() == CLASS_HUNTER && victim)
	{
		m_hunterTargetlive = victim->GetMaxHealth();
		m_hunterTargetType = victim->GetTypeId();
		m_hunterTargetLevel = victim->getLevel();
		m_hunterGuardian = victim->IsPetGuardianStuff();
		m_huntertargetgear = (victim->ToPlayer()) ? victim->ToPlayer()->GetAverageItemLevel() : 1;
	}

	// Asignar datos de la victima  
	TypeID victim_type = (victim) ? victim->GetTypeId() : m_hunterTargetType;
	uint32 maxTargetLive = (victim) ? victim->GetMaxHealth() : m_hunterTargetlive;
	uint32 victimgearlevel = (victim && victim->ToPlayer()) ? victim->ToPlayer()->GetAverageItemLevel() : m_huntertargetgear;
	uint8 victimLevel = (victim) ? victim->getLevel() : m_hunterTargetLevel;
	bool targetGuardian = (victim) ? victim->IsPetGuardianStuff() : m_hunterGuardian;
	// Caster TypeID
	TypeID caster_type = this->GetTypeId();
	// Only consider < 90 level
	if ((victimLevel == 90) || this->getLevel() == 90)
		return value;
	// Recalculated value	
	uint32 finalValue = value;
	//Heal control
	if (!damage){
		if ((caster_type == TYPEID_PLAYER  && victim_type == TYPEID_PLAYER))
		{
			uint32 heal_a = uint32(maxTargetLive * 0.5f);
			if (value >= heal_a)
				finalValue = heal_a;
		}
	}
		...
		float multiplier = 1;
		uint32 castergearlevel = (caster_type == TYPEID_PLAYER) ? ToPlayer()->GetAverageItemLevel() : 1;

		if (castergearlevel <= victimgearlevel && victimgearlevel > 0)
		{
			float percentequalgear_a = (float)castergearlevel / victimgearlevel;
			//if (percentequalgear_a >= 0.93f)
			//	multiplier = 0.21f;		
			//if (percentequalgear_a >= 0.73f)
			//	multiplier = 0.19f;
			//if (percentequalgear_a >= 0.53f)
			//	multiplier = 0.18f;
			//if (percentequalgear_a < 0.53f)
			//	multiplier = 0.15f;
			//if (percentequalgear_a = 0)
			//	multiplier = 0.0001f;
			// y = 0,7803x3 - 1,4407x2 + 0,892x - 0,0003

			multiplier = (float(0.7803f * std::pow(percentequalgear_a, 3) - 1.4407f * std::pow(percentequalgear_a, 2) + 0.892f * percentequalgear_a + 0.0001f));
		}
		else if (castergearlevel > 0)
		{
			float percentequalgear_b = (float)victimgearlevel / castergearlevel;
			//if (percentequalgear_b >= 0.93f)
			//	multiplier = 0.21f;
			//if (percentequalgear_b >= 0.83f)
			//	multiplier = 0.24f;
			//if (percentequalgear_b >= 0.73f)
			//	multiplier = 0.27f;
			//if (percentequalgear_b >= 0.63f)
			//	multiplier = 0.30f;
			//if (percentequalgear_b >= 0.53f)
			//	multiplier = 0.33f;
			// y = -0,3x + 0,489

			multiplier = float(-0.3f * percentequalgear_b + 0.489f);
		}
		else
		{
			multiplier = 0.1f;
		...
		//Control de level
		if (victimLevel > this->getLevel()){
			// 0.1 * 0.25
			float leveldiff = float(std::abs(victimLevel - this->getLevel()) * 0.025f);
			multiplier -= leveldiff;
		}
		else if (this->getLevel() > victimLevel){
			// 0.15 * 0.25
			float leveldiff = float(std::abs(victimLevel - this->getLevel()) * 0.0375);
			multiplier += leveldiff;
		}
		...
		if (caster_type == TYPEID_PLAYER  && victim_type == TYPEID_PLAYER)
		{
			uint32 damag_a = uint32(maxTargetLive * multiplier);
			if (value >= damag_a)
				finalValue = damag_a;
		}
		if ((this->IsPetGuardianStuff() || (this->IsControlledByPlayer() && caster_type != TYPEID_PLAYER) || (IsPlayerPet(this->GetGUID()))) && victim_type == TYPEID_PLAYER)
		{
			
			uint32 damag_b = uint32(maxTargetLive * (0.08f));
			
			if (this->GetEntry() == 27829){
				damag_b = uint32(maxTargetLive * (0.04f));
			}
		...
		if (caster_type == TYPEID_PLAYER && (targetGuardian || (IsPlayerPet(this->GetGUID()))))
		{
			uint32 damag_b = uint32(maxTargetLive * (0.20f));
			if (value >= damag_b)
				finalValue = damag_b;
		}
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (spellProto->Id == 50273) // Arcane Barrage
        return pdamage;
    ...
    // Apply PvP Power damage bonus
    if (pdamage > 0 && GetTypeId() == TYPEID_PLAYER && (victim->GetTypeId() == TYPEID_PLAYER || (victim->GetTypeId() == TYPEID_UNIT && victim->GetOwner() && victim->GetOwner()->ToPlayer())))
    {
        float PvPPower = ToPlayer()->GetPvpPowerDamagePct();
    ...
    if (GetTypeId() == TYPEID_PLAYER && pdamage != 0 && getClass() == CLASS_ROGUE && ToPlayer()->GetSpecializationId(ToPlayer()->GetActiveSpec()) == SPEC_ROGUE_ASSASSINATION)
    {
        Item* mainItem = ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
        Item* offItem = ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
    ...
    // Bonus damage while in caster form
    if (GetTypeId() == TYPEID_PLAYER && HasAura(77219) && !HasAura(103958))
    {
        float Mastery = GetFloatValue(PLAYER_MASTERY);
		AddPct(DoneTotalMod, Mastery);
    }
    ...
    float ModByDamagePercentDone = 0;
        ...
        for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
        {
            ...
            if ((*i)->GetMiscValue() & spellProto->GetSchoolMask())
            {
                ...
                if ((*i)->GetSpellInfo()->EquippedItemClass == -1)
                    ModByDamagePercentDone += (*i)->GetAmount();
    ...
    ModByDamagePercentDone /= 100;
    DoneTotalMod += ModByDamagePercentDone;
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (spellProto->Id)
            {
                ...
                case 100214:
                    // Fury, SPELL_AURA_276
                    if (constAuraPtr aur = GetAura(97235))
                    ...
                    case SPELLFAMILY_SHAMAN:
                    {
                        switch (spellProto->Id)
                        {
                            ...
                            case 77451: // Lava Burst mastery
                                ...
                                return victim->HasAura(53480) ? 0 : 100.0f;
                                break;
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    // No bonus for Atonement or Eminence
    if (spellProto->Id == 81751 || spellProto->Id == 117895)
        return healamount;

    // Seal of insight
    if (spellProto->Id == 20167)
        return healamount;
    ...
    // Apply PvP Power healing bonus
    if (healamount > 0 && GetTypeId() == TYPEID_PLAYER && (victim->GetTypeId() == TYPEID_PLAYER || (victim->GetTypeId() == TYPEID_UNIT && victim->GetOwner() && victim->GetOwner()->ToPlayer())))
    {
        if (victim->HasAura(134735))
        {
            float PvPPower = ToPlayer()->GetPvpPowerHealingPct();
            AddPct(DoneTotalMod, PvPPower);
        }
    }
...
uint32 Unit::SpellHealingBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
	...
	if (auto auraEffect = GetAuraEffect(110310, EFFECT_0))
	{
		if (auto effAmount = auraEffect->GetAmount())
			AddPct(TakenTotalMod, -effAmount);
	}
	...
	// Earthliving should increase heal based on player level, this  scaling is based on regresions fron spell heal data vs player level
	if (caster->getClass() == CLASS_SHAMAN && caster->ToPlayer()->GetSpecializationId(caster->ToPlayer()->GetActiveSpec()) == SPEC_SHAMAN_RESTORATION && caster->HasAura(52007))
	{
		uint8 casterLevel = caster->getLevel();
		// From level 30 to level 56
		if (casterLevel >= 30 && casterLevel <= 56)
			healamount += 1.0189f  * casterLevel + 2.7418f;
		// From level 57 to level 67
		if (casterLevel >= 57 && casterLevel <= 67)
			healamount += 2.2636f * casterLevel - 52.164f;
		// From level 68 to level 80
		if (casterLevel >= 68 && casterLevel <= 80)
			healamount += 6.5934f * casterLevel - 310.76f;
		// From level 81 to level 85
		if (casterLevel >= 81 && casterLevel <= 85)
			healamount += 83.3f * casterLevel - 6208.7f;
		// From level 86 to level 90, this  polinomical regresion is grade 2 for minimize errors, < 1% error
		if (casterLevel >= 86 && casterLevel <= 90)
			healamount += 31.214f * std::pow(casterLevel, 2) - 5119.4f * casterLevel + 210785;
	}

    // Glyph of Recovery
    if (caster->getClass() == CLASS_ROGUE && spellProto->Id != 73651 && caster->HasAura(146625) && caster->HasAura(73651))
	...
	try
	{
		...
		uint32 mHealingGetSize = mHealingGet.size() + 1;

		for (AuraEffectList::const_iterator i = mHealingGet.begin(); i != mHealingGet.end(); ++i)
		{
			if (loopCounter > mHealingGetSize)
			{
				sLog->outInfo(LOG_FILTER_WORLDSERVER, "Loop prevent on Unit::SpellHealingBonusTaken loopCounter: [%u] mHealingGetSize: [%u]", loopCounter, mHealingGetSize);
				break;
			}
			loopCounter++;

			if (!(*i)->IsAffectingSpell(spellProto))
				if ((*i)->HasSpellClassMaskSafe())
					continue;

			uint32 MiscValueMask = (*i)->GetMiscValue();

			if (MiscValueMask & 1) // MiscValue |= 1 increase healing only from the original caster
				if (caster->GetGUID() && (caster->GetGUID() != (*i)->GetCasterGUID()))
					continue;

			if (MiscValueMask & 2) // MiscValue |= 2 only works on direct healings
				if (damagetype == DOT)
					continue;

			if (MiscValueMask & 4) // MiscValue |= 4 only works on healings on time
				if (damagetype != DOT)
					continue;

			AddPct(TakenTotalMod, (*i)->GetAmount());

			if (MiscValueMask & 8) // MiscValue |= 8 remove aura on first use
				if ((*i)->GetSpellInfo()->Id)
					RemoveAurasDueToSpell((*i)->GetSpellInfo()->Id);
		}
	}
	catch (...)
	{
		sLog->outInfo(LOG_FILTER_WORLDSERVER, "Unknwon exception catch at Unit::SpellHealingBonusTaken");
	}
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
		...
		// Check if we are ever using mana - PaperDollFrame.lua		
		if (GetPowerIndexByClass(POWER_MANA, getClass()) != MAX_POWERS && getLevel() > 84 && getLevel() < 86)// Only for level 85
...
uint32 Unit::MeleeDamageBonusDone(Unit* victim, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetSpecializationId(ToPlayer()->GetActiveSpec()) == SPEC_WARRIOR_ARMS && attType == BASE_ATTACK && HasAura(46924))
        AddPct(DoneTotalMod, 60);

    // Might of the Frozen Wastes - 81333
    // Increase damage dealt by two handed weapons by 30%
    if (pdamage > 0 && GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetSpecializationId(ToPlayer()->GetActiveSpec()) == SPEC_DK_FROST && ToPlayer()->IsTwoHandUsed() && HasAura(81333))
    {
    ...
    // Sudden Death - 29725
    if (GetTypeId() == TYPEID_PLAYER && ToPlayer()->GetSpecializationId(ToPlayer()->GetActiveSpec()) == SPEC_WARRIOR_ARMS && (attType == BASE_ATTACK || attType == OFF_ATTACK || spellProto->Id == 76858) && HasAura(29725))
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && victim && pdamage != 0 && (attType == BASE_ATTACK || attType == OFF_ATTACK))
    {
        bool flag = HasAura(76838) && (spellProto && spellProto->Id != 76858 && spellProto->Id != 26654 || !spellProto);
...
void Unit::Mount(uint32 mount, uint32 VehicleId, uint32 creatureEntry)
{
    ...
    // call this method to correctly handle battleground thinks like remove obrs, flags etc.
    if (GetTypeId() == TYPEID_PLAYER)
        ToPlayer()->HandleBGDropEvent();
}
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
	...
	if (target == nullptr)
	{
		sLog->OutSpecialLog("Unit::_IsValidAttackTarget, target is null!");
		return false;
	}
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, uint32 absorb, SpellInfo const* procAura)
{
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK && target)
    {
        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT|PROC_EX_PARRY|PROC_EX_DODGE|PROC_EX_BLOCK))
        {
            ...
            else // For attacker
            {
                ...
                if (procExtra & PROC_EX_DODGE && GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_WARRIOR && this->ToPlayer()->GetSpecializationId(this->ToPlayer()->GetActiveSpec()) == SPEC_WARRIOR_ARMS)
                {
    ...
    // Dematerialize
    if (target && target->GetTypeId() == TYPEID_PLAYER && procSpell && (procSpell->GetAllEffectsMechanicMask() & (1 << MECHANIC_STUN)) && target->HasAura(122464) && !isVictim)
    {
    ...
    if (GetTypeId() == TYPEID_PLAYER && procSpell && (procSpell->Id == 84617 || procSpell->Id == 1752) && HasAura(84654))
    {
        insightCount++;
    ...
    // Hack Fix Frenzy
    if (GetTypeId() == TYPEID_UNIT && isHunterPet() && GetOwner() && GetOwner()->ToPlayer() && ToPet()->IsPermanentPetFor(GetOwner()->ToPlayer()) && !procSpell && GetOwner()->HasAura(19623))
        if (roll_chance_i(40))
            CastSpell(this, 19615, true);

    // Hack Fix for Invigoration
    if (GetTypeId() == TYPEID_UNIT && GetOwner() && GetOwner()->ToPlayer() && damage > 0 && ToPet() && ToPet()->IsPermanentPetFor(GetOwner()->ToPlayer()) && GetOwner()->HasAura(53253))
        if (roll_chance_i(15))
            GetOwner()->EnergizeBySpell(GetOwner(), 53253, 20, POWER_FOCUS);

    // Fix Drop charge for Killing Machine
    if (GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_DEATH_KNIGHT && procSpell && HasAura(51124))
    {
        if (haveOffhandWeapon())
        {
    ...
    if (procAura && procAura->Id == 89523)
    {
        target->DealDamage(this, damage, NULL, SPELL_DIRECT_DAMAGE, procSpell->GetSchoolMask(), procSpell, true);
        RemoveAurasDueToSpell(89523);
    }
...
void Unit::FilterTargets(std::list<Unit*> &targetList, float range, uint32 mask, uint32 maxTargets, Unit* exclude, ListOrder listOrder, float arcAngle, int32 aura, uint64 auraOwnerGUID, bool ascendingOrder, ArcPosition arcPos) const
{
    Unit* caster = ((Unit*)this);

    WoWSource::AnyUnitInObjectRangeCheck u_check(this, range);
    WoWSource::UnitListSearcher<WoWSource::AnyUnitInObjectRangeCheck> searcher(this, targetList, u_check);
    VisitNearbyObject(range, searcher);

    if (maxTargets == NULL) // "NULL" Means no limit
        maxTargets = -1;

    if (!targetList.empty())
    {
        if (exclude != NULL) // "NULL" Means no exclusions
            targetList.remove(exclude);

        for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
        {
            if ((*tIter)->isSpiritService())
                targetList.erase(tIter++);
            else
                ++tIter;
        }

        if (!(mask & INCLUDE_CRITTERS))
        {
            for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
            {
                if ((*tIter)->GetCreatureType() == CREATURE_TYPE_CRITTER)
                    targetList.erase(tIter++);
                else
                    ++tIter;
            }
        ...
        if (mask & EXCLUDE_ALLIES)
        {
            for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
            {
                if (IsFriendlyTo(*tIter))
                    targetList.erase(tIter++);
                else
                    ++tIter;
            }

            if (!(mask & INCLUDE_NOT_VALID_ATTACK))
            {
                for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
                {
                    if (!IsValidAttackTarget(*tIter))
                        targetList.erase(tIter++);
                    else
                        ++tIter;
                }
        ...
        if (mask & EXCLUDE_ENEMIES)
        {
            for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
            {
                if (IsHostileTo(*tIter))
                    targetList.erase(tIter++);
                else
                    ++tIter;
            }

            if (!(mask & INCLUDE_NOT_VALID_ASSIST))
            {
                for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
                {
                    if (!IsValidAssistTarget(*tIter))
                        targetList.erase(tIter++);
                    else
                        ++tIter;
                }
        ...
        if (mask & ONLY_RAID_MEMBERS)
        {
            for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
            {
                if (!IsInRaidWith(*tIter))
                    targetList.erase(tIter++);
                else
                    ++tIter;
            }
        ...
        if (mask & ONLY_ARC)
        {
            for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
            {
                ...
                switch (arcPos)
                {
                    case Front:
                    {
                        if (!isInFront(*tIter, M_PI / (180 / arcAngle)))
                            targetList.erase(tIter++);
                        else
                            ++tIter;
                    ...
                    case Back:
                    {
                        if (!isInBack(*tIter, M_PI / (180 / arcAngle)))
                            targetList.erase(tIter++);
                        else
                            ++tIter;
                    ...
                    case Sides:
                    {
                        ...
                        if (isInFront(*tIter, ((180 - arcAngle) * M_PI) / 180) || isInBack(*tIter, ((180 - arcAngle) * M_PI) / 180))
                            targetList.erase(tIter++);
                        else
                            ++tIter;
        ...
        if (mask & HAS_SPECIFIC_AURA)
        {
            for (std::list<Unit*>::iterator tIter = targetList.begin(); tIter != targetList.end();)
            {
                for (auto itr : targetList)
                {
                    if (!itr->HasAura(aura, auraOwnerGUID))
                        targetList.erase(tIter++);
                    else
                        ++tIter;
                }
        ...
        switch (listOrder)
        {
            case RandomOrderPred:
                if (maxTargets != NULL)
                    WoWSource::RandomResizeList(targetList, maxTargets);
                break;
            case PowerPctOrderPred:
                targetList.sort(WoWSource::PowerPctOrderPred(getPowerType(), ascendingOrder));
                break;
            case UnitDistanceCompareOrderPred:
                targetList.sort(WoWSource::UnitDistanceCompareOrderPred(this, ascendingOrder));
                break;
            case HealthPctOrderPred:
                targetList.sort(WoWSource::HealthPctOrderPred(ascendingOrder));
                break;
            case DistanceCompareOrderPred:
                targetList.sort(WoWSource::DistanceCompareOrderPred(this, ascendingOrder));
                break;
            case DistanceOrderPred:
                targetList.sort(WoWSource::DistanceOrderPred(caster));
                break;
            default:
                break;
        }
...
void Unit::ApplyAttackTimePercentMod(WeaponAttackType att, float val, bool apply)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        case BASE_ATTACK:
        case OFF_ATTACK:
            ToPlayer()->UpdateMeleeHaste();
            break;
        case RANGED_ATTACK:
            ToPlayer()->UpdateRangeHaste();
            break;
        }
    }
...
uint32 Unit::GetCastingTimeForBonus(SpellInfo const* spellProto, DamageEffectType damagetype, uint32 CastingTime) const
{
    ...
    for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
    {
        if (spellProto->Effects[j].Effect == SPELL_EFFECT_HEALTH_LEECH ||
            ...
            (spellProto->Effects[j].Effect == SPELL_EFFECT_APPLY_AURA_ON_PET && spellProto->Effects[j].ApplyAuraName == SPELL_AURA_PERIODIC_LEECH))
        {
            CastingTime /= 2;
            break;
        }
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, AuraPtr aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    if (!sSpellMgr->IsSpellProcEventCanTriggeredBy(spellProcEvent, EventProcFlag, procSpell, procFlag, procExtra, active))
    {
        ...
        if (spellProto && spellProto->Id == 117896 && procSpell && procSpell->Id == 17962 && procExtra && (procExtra & PROC_EX_ABSORB))
            ...
            (procSpell->Id == 108853 || procSpell->Id == 11366 || procSpell->Id == 11129)) // Inferno Blast, Combustion and Pyroblast can Trigger Pyroblast! if damage are absorbed
            return true;
        else if (spellProto && spellProto->Id == 93399 && procSpell &&
            (procSpell->Id == 8921 || procSpell->Id == 93402) && (procExtra & PROC_EX_ABSORB)) // Moonfire and Sunfire can Trigger Shooting Stars if damage are absorbed
            return true;
...
void Unit::Kill(Unit* victim, bool durabilityLoss, SpellInfo const* spellProto)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && victim->getClass() == CLASS_PRIEST && !victim->HasAura(47788))
    {
...
AuraPtr Unit::AddAura(uint32 spellId, Unit* target, uint64 casterGuid /*= 0*/)
{
    if (!target)
        return NULLAURA;
    ...
    return AddAura(spellInfo, MAX_EFFECT_MASK, target, casterGuid);
}

AuraPtr Unit::AddAura(SpellInfo const* spellInfo, uint32 effMask, Unit* target, uint64 casterGuid /*= 0*/)
{
    if (!spellInfo)
        return NULLAURA;
    ...
    AuraPtr aura = Aura::TryRefreshStackOrCreate(spellInfo, effMask, target, this, spellInfo->spellPower, nullptr, nullptr, casterGuid);
    if (aura != NULLAURA)
    {
...
void Unit::ApplyResilience(Unit const* victim, int32* damage) const
{
    ...
    if (IsVehicle() && !HasAura(115034) && GetTypeId() != TYPEID_PLAYER)
        return;
...
float Unit::MeleeSpellMissChance(const Unit* victim, WeaponAttackType attType, uint32 spellId) const
{
    ...
    if (victim->getLevel() > getLevel())
    {
        ...
        if (level_diff <= 2)
            ...
            missChance = (missChance - 3) + level_diff * 2.0f;
    }
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    {
        WorldPacket data(SMSG_SPLINE_MOVE_UNROOT);
...
uint32 Unit::GetHealingDoneInPastSecs(uint32 secs)
{
    ...
    for (auto itr : m_healDamage)
    {
        if (itr.m_type != HEAL_DONE_LOG)
            continue;

        if ((getMSTime() - itr.m_time) <= (secs * IN_MILLISECONDS))
            heal += itr.m_value;
    }
...
void Unit::RemoveSoulSwapDOT(Unit* target)
{
	...
	m_SoulSwapTarget = target->GetGUID();
	AuraEffectList const* m_periodic_copy = &target->GetAuraEffectsByType(SPELL_AURA_PERIODIC_DAMAGE);	
	for (auto itr : *m_periodic_copy)
	{
		if (!(itr)) // prevent crash
			continue;

		if ((itr)->GetSpellInfo()->SpellFamilyName != SPELLFAMILY_WARLOCK ||
			(itr)->GetCasterGUID() != GetGUID()) // only warlock spells
			continue;

		AuraPtr l_Aura = (itr)->GetBase();
...
void Unit::ApplySoulSwapDOT(Unit* caster, Unit* target)
{
	...
	{
		bool alreadyExist = target->HasAura(l_Aura.m_ID);
		if (AuraPtr l_NewAura = AddAura(l_Aura.m_ID, target))
		{
			int32 l_Duration = l_Aura.m_Duration;
			int32 l_MaxDuration = l_Aura.m_MaxDuration;

			/// Pandemic should works too on Soul Swap
			if (AuraPtr l_OldAura = target->GetAura(l_Aura.m_ID, GetGUID()))
			{
...
int32 Unit::GetHighestExclusiveSameEffectSpellGroupValue(constAuraEffectPtr aurEff, AuraType auraType, int32 miscValue /*= NULL*/, int32 miscValueB /*= NULL*/) const
{
    int32 val = 0;
    SpellSpellGroupMapBounds spellGroup = sSpellMgr->GetSpellSpellGroupMapBounds(aurEff->GetSpellInfo()->Id);
    for (SpellSpellGroupMap::const_iterator itr = spellGroup.first; itr != spellGroup.second; ++itr)
    {
        if (sSpellMgr->GetSpellGroupStackRule(itr->second) == SPELL_GROUP_STACK_RULE_EXCLUSIVE_SAME_EFFECT)
        {
            ...
            for (AuraEffectList::const_iterator auraItr = auraEffList.begin(); auraItr != auraEffList.end(); ++auraItr)
            {
                ...
                if (aurEff != (*auraItr) && (miscValue == NULL || (*auraItr)->GetMiscValue() == miscValue) && 
                    (miscValueB == NULL || (*auraItr)->GetMiscValueB() == miscValueB) &&
