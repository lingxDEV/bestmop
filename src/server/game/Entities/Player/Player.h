...
enum PlayerLoginQueryIndex
{
	...
	PLAYER_LOGIN_QUERY_OLD_LOADCRITERIAPROGRESS,
	PLAYER_LOGIN_QUERY_OLD_LOADACCOUNTCRITERIAPROGRESS,
	...
	PLAYER_LOGIN_QUERY_QUEST_CURRENCY,
	...
	PLAYER_LOGIN_QUERY_LOADWEEKENDBG,
...
enum AtLoginFlags
{
	...
	AT_LOGIN_CONVERT_TO_BATTLE_PETS = 0x400,
	AT_LOGIN_FIX_ACHIEVEMENT	= 0x800,
	AT_LOGIN_FIX_ARCHEAOLOGY	 = 0x1000,
	AT_LOGIN_RESURRECT			= 0x2000,
};
...
enum PlayerFlags
{
	...
	PLAYER_FLAGS_BATTLE_PET_ENABLED		= 0x01000000,       // Unlock battle pet slot
...
enum PlayerDelayedOperations
{
    ...
    DELAYED_BG_UPDATE_ZONE      = 0x040,                     ///< Flag to run UpdateZone() function to send current arena season
    DELAYED_BG_NAME_RESTORE     = 0x080,                     ///< Flag to send old SMSG_QUERY_NAME_OPCODE to all players within dist
    DELAYED_SPECTATOR_REMOVE    = 0x100,
    DELAYED_PET_BATTLE_INITIAL  = 0x200,
    DELAYED_END
};

// Player summoning auto-decline time (in secs)
#define MAX_PLAYER_SUMMON_DELAY                   (2*MINUTE)
#define MAX_MONEY_AMOUNT               (UI64LIT(9999999999)) // One million gold. Guild limitation too. @TODO: Move this restriction to worldserver.conf, default to this value, hardcap at uint64.max
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        void BuildPlayerChat(WorldPacket* data, uint8 msgtype, const std::string& text, uint32 language, const char* addonPrefix = NULL, const std::string& channel = "", uint64 receiverGUID = 0, uint8 chatTag = CHAT_TAG_NONE, uint32 achievementId = 0);
        ...
        */
		...
		void ModifyCurrency(uint32 id, int32 count, bool printLog = true, bool ignoreMultipliers = false, bool ignoreLimit = false, bool arenaReawrd = false);
        ...
        void CompleteQuest(uint32 quest_id, bool noRewards = false);
        void IncompleteQuest(uint32 quest_id);
        void RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce = true, bool noRewards = false);
        ...
        uint32 GetSpecializationId() const { return GetSpecializationId(GetActiveSpec()); }
        ...
        void ResetSpec();
        void LearnSpecializationByTab(uint32 const tab);
        void LearnSpecializationById(uint32 const specializationId);
        void LearnSpecialization(uint32 const specializationId, uint32 const specializationSpell);

		Stats GetHighestStat() const;
        ...
        void SendSpellCooldown(uint32 spell_id, uint32 cooldown);
        void SendSpellCooldown(ByteBuffer& buffer);
        ...
        {
            ASSERT(slot < MAX_PVP_SLOT);

            /*if (value > 3500)
            {
                ACE_Stack_Trace trace;
                sLog->outError(LOG_FILTER_BATTLEGROUND, "Suspiciously high personal rating. Rating: %u, Slot: %u, Player: %u, Trace log: %s", value, slot, GUID_LOPART(GetGUID()), trace.c_str());
            }*/

            UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_PERSONAL_RATING, value, Arena::GetTypeBySlot(slot));
        ...
        void SetArenaMatchMakerRating(uint8 slot, uint32 value)
        {
            ...
            {
                ACE_Stack_Trace trace;
                sLog->outError(LOG_FILTER_BATTLEGROUND, "Suspiciously high match maker rating. Rating: %u, Slot: %u, Player: %u, Trace log: %s", value, slot, GUID_LOPART(GetGUID()), trace.c_str());
            }*/
        ...
        void IncrementSeasonWins(uint8 slot) { ASSERT(slot < MAX_PVP_SLOT); ++m_SeasonWins[slot]; }
        void FinishWeek();
        ...
        float GetRangeHastePct(bool total = true);
        float GetSpellHastePct(bool total = true);
        float GetMeleeHastePct(bool total = true);

        void UpdateRangeHaste();
        void UpdateSpellHaste();
        void UpdateMeleeHaste();

        void UpdateFlatModCritValue();
        ...
        float GetPvpPowerHealingPct() const { return GetFloatValue(PLAYER_FIELD_PVP_POWER_HEALING); }
        float GetPvpPowerDamagePct() const { return GetFloatValue(PLAYER_FIELD_PVP_POWER_DAMAGE); }
        ...
        */
        ...
        uint32 GetGearSpecializationSpell() const;
        void UpdateGearSpecialization();
        void ResetGearSpecizalization(Item const* byItem = nullptr);

        uint8 AllItemFitToSubclass(ItemSubclassArmor subClass);
		...
		BattlePet::PetJournal* GetPetJournal() { return m_petJournal; }
		BattlePet::PetJournal const* GetPetJournal() const { return m_petJournal; }
        ...
        void PlayScene(uint32 p_SceneID, WorldObject* p_Spectator);
		...
		void SetWargameInvite(bool p_Invited) { m_IsInvitedToWargame = p_Invited; }
		bool IsInvitedToWargame() const { return m_IsInvitedToWargame; }

        void ForcedSaveCurrency(SQLTransaction& trans) { _SaveCurrency(trans); }

		void lock() { m_mutex.try_lock(); }
		void unlock() { m_mutex.unlock(); }
		bool HasSpellModTakingSpell() { if (m_spellModTakingSpell) return true; else return false; }
...
//        Spell* m_spellModTakingSpell;  // Spell for which charges are dropped in spell::finish
    ...
    private:
		...
		BattlePet::PetJournal* m_petJournal;
		...
		// RoG VIP system
		float m_xp_rate;         // Rate for all XP gain
		float m_reputation_rate; // Rate for all rep gain 
		float m_money_rate;      // Rate for all money gain
		uint8 m_skill_rate;      // Rate for all skill gain
