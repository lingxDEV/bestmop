...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_UPD_EXPIRED_ACCOUNT_BANS, "UPDATE account_banned SET active = 0 WHERE active = 1 AND unbandate<>bandate AND unbandate<=UNIX_TIMESTAMP()", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_AUTO_BANNED, "INSERT INTO account_banned (`id`, `bandate`, `unbandate`, `bannedby`, `banreason`, `active`) VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'Trinity realmd', 'Failed login autoban', 1)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_BANNED, "DELETE FROM account_banned WHERE id = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_SESSIONKEY, "SELECT a.sessionkey, a.id, aa.gmlevel  FROM account a LEFT JOIN mop548_account_access aa ON (a.id = aa.id) WHERE username = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_LOGONCHALLENGE, "SELECT a.sha_pass_hash, a.id, a.locked, a.last_ip, aa.gmlevel, a.v, a.s, a.token_key_548 FROM account a LEFT JOIN mop548_account_access aa ON (a.id = aa.id) WHERE a.username = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_INS_LOG_IP, "INSERT IGNORE INTO mop548_account_log_ip (`accountid`, `ip`, `date`) VALUES (?, ?, NOW())", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_INFO_BY_NAME, "SELECT id, sessionkey, last_ip, locked, v, s, expansion, mutetime, locale, recruiter, os, hasBoost FROM account WHERE username = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_BANNED, "INSERT INTO account_banned (`id`, `bandate`, `unbandate`, `bannedby`, `banreason`, `active`) VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, ?, ?, 1)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_BANNED, "SELECT 1 FROM account_banned WHERE id = ? AND active = 1 AND bandate = unbandate", CONNECTION_SYNCH);
	...
	PREPARE_STATEMENT(LOGIN_UPD_ACCOUNT_BOOST, "UPDATE account SET hasBoost = ? WHERE id = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_UPD_ACCOUNT_LOCK, "UPDATE account SET locked = ? WHERE id = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_LOG, "INSERT INTO mop548_logs (time, realm, type, level, string) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(LOGIN_UPD_WOD_PASSWORD, "UPDATE account SET v = 0, s = 0, sha_pass_hash_wod623 = ?, bnet2_pass_hash = ?, bnet2_salt = ?, username_wod623 = ? WHERE id = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(LOGIN_DEL_OLD_LOGS, "DELETE FROM mop548_logs WHERE (time + ?) < ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_ACCESS, "DELETE FROM mop548_account_access WHERE id = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_ACCESS_BY_REALM, "DELETE FROM mop548_account_access WHERE id = ? AND (RealmID = ? OR RealmID = -1)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_ACCESS, "INSERT INTO mop548_account_access (id,gmlevel,RealmID,comment) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_GET_ACCOUNT_ID_BY_USERNAME, "SELECT id FROM account WHERE username = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_GET_ACCOUNT_ACCESS_GMLEVEL, "SELECT gmlevel FROM mop548_account_access WHERE id = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_GET_GMLEVEL_BY_REALMID, "SELECT gmlevel FROM mop548_account_access WHERE id = ? AND (RealmID = ? OR RealmID = -1)", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_PINFO, "SELECT a.username, aa.gmlevel, a.email, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), a.mutetime FROM account a LEFT JOIN mop548_account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) WHERE a.id = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_GM_ACCOUNTS, "SELECT a.username, aa.gmlevel FROM account a, mop548_account_access aa WHERE a.id=aa.id AND aa.gmlevel >= ? AND (aa.realmid = -1 OR aa.realmid = ?)", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_INFO, "SELECT a.username, a.last_ip, aa.gmlevel, a.expansion FROM account a LEFT JOIN mop548_account_access aa ON (a.id = aa.id) WHERE a.id = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_ACCESS_GMLEVEL_TEST, "SELECT 1 FROM mop548_account_access WHERE id = ? AND gmlevel > ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_ACCESS, "SELECT a.id, aa.gmlevel, aa.RealmID FROM account a LEFT JOIN mop548_account_access aa ON (a.id = aa.id) WHERE a.username = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_SET_DUMP, "UPDATE mop548_transferts SET dump = ? ,revision = ? WHERE id = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_ADD_TRANSFERTS_LOGS, "INSERT INTO mop548_transferts_logs (`id`, `account`, `perso_guid`, `from`, `to`, `dump`) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);

    PREPARE_STATEMENT(LOGIN_INS_CHAR_SPELL, "INSERT INTO mop548_account_spell (accountId, spell, active, disabled) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_CHARACTER_SPELL, "SELECT spell, active, disabled FROM mop548_account_spell WHERE accountId = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_CHAR_SPELL_BY_SPELL, "DELETE FROM mop548_account_spell WHERE spell = ? AND accountId = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_CHAR_SPELL, "DELETE FROM mop548_account_spell WHERE accountId = ?", CONNECTION_ASYNC);

    PREPARE_STATEMENT(LOGIN_UPD_ACCOUNT_PREMIUM, "UPDATE mop548_account_premium SET active = 0 WHERE active = 1 AND unsetdate<=UNIX_TIMESTAMP() AND unsetdate<>setdate", CONNECTION_SYNCH);
	
	PREPARE_STATEMENT(LOGIN_UPD_LAST_ATTEMPT_IP, "UPDATE account SET last_attempt_ip_548 = ? WHERE id = ?", CONNECTION_ASYNC);
}
