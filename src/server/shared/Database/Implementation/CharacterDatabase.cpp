...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    "health, power1, power2, power3, power4, power5, instance_id, speccount, activespec, specialization1, specialization2, exploredZones, equipmentCache, knownTitles, actionBars, currentpetslot, petslotused, grantableLevels, resetspecialization_cost, resetspecialization_time FROM characters WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHARACTER_INVENTORY, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, reforgeId, transmogrifyId, upgradeId, durability, playedTime, text, bag, slot, "
    ...
    PREPARE_STATEMENT(CHAR_SEL_GUILD_MEMBER, "SELECT guildid, guild_rank FROM guild_member WHERE guid = ?", CONNECTION_BOTH);
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHARACTER_ARENA_DATA, "SELECT rating0, bestRatingOfWeek0, bestRatingOfSeason0, matchMakerRating0, weekGames0, weekWins0, prevWeekWins0, seasonGames0, seasonWins0, rating1, bestRatingOfWeek1, bestRatingOfSeason1, matchMakerRating1, weekGames1, weekWins1, prevWeekWins1, seasonGames1, seasonWins1, rating2, bestRatingOfWeek2, bestRatingOfSeason2, matchMakerRating2, weekGames2, weekWins2, prevWeekWins2, seasonGames2, seasonWins2, rating3, bestRatingOfWeek3, bestRatingOfSeason3, matchMakerRating3, weekGames3, weekWins3, prevWeekWins3, seasonGames3, seasonWins3 FROM character_arena_data WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_MAILITEMS, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, reforgeId, transmogrifyId, upgradeId, durability, playedTime, text, item_guid, itemEntry, owner_guid FROM mail_items mi JOIN item_instance ii ON mi.item_guid = ii.guid WHERE mail_id = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_SEL_AUCTION_ITEMS, "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, reforgeId, transmogrifyId, upgradeId, durability, playedTime, text, itemguid, itemEntry FROM auctionhouse ah JOIN item_instance ii ON ah.itemguid = ii.guid", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(CHAR_REP_ITEM_INSTANCE, "REPLACE INTO item_instance (itemEntry, owner_guid, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, reforgeId, transmogrifyId, upgradeId, durability, playedTime, text, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_ITEM_INSTANCE, "UPDATE item_instance SET itemEntry = ?, owner_guid = ?, creatorGuid = ?, giftCreatorGuid = ?, count = ?, duration = ?, charges = ?, flags = ?, enchantments = ?, randomPropertyId = ?, reforgeId = ?, transmogrifyId = ?, upgradeId = ?, durability = ?, playedTime = ?, text = ? WHERE guid = ?", CONNECTION_ASYNC);
	...
	// Character Boost
	PREPARE_STATEMENT(CHAR_UPD_CHARACTER_BOOST, "UPDATE characters SET race = ?, money = money + ?, level = ?, position_x = ?, position_y = ?, position_z = ?, orientation = ?, map = ?, talenttree = ?,"
		"trans_x = 0, trans_y = 0, trans_z = 0, trans_o = 0, transguid = 0, at_login = at_login & ~ ?, activespec = 0, equipmentCache = ? WHERE guid = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_CHAR_INVENTORY_ITEM_GUID_UNTIL_BAG_SLOT, "SELECT item FROM character_inventory WHERE bag = ? AND slot < ? AND guid = ?", CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_SEL_CHAR_TALENT, "SELECT spell FROM character_talent WHERE guid = ?", CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_DEL_CHAR_INVENTORY_UNTIL_BAG_SLOT, "DELETE FROM character_inventory WHERE bag = ? AND slot < ? AND guid = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_INS_CHAR_INVENTORY, "INSERT INTO character_inventory (guid, bag, slot, item) VALUES (?, ?, ?, ?)"
    ...
    PREPARE_STATEMENT(CHAR_INS_GUILD_MEMBER, "INSERT INTO guild_member (guildid, guid, guild_rank, pnote, offnote) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_UPD_GUILD_MEMBERS_RANK, "UPDATE guild_member SET guild_rank = ? WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC); // 0: uint32
    // 0: uint32, 1: uint8, 3: string, 4: uint32, 5: uint32
    PREPARE_STATEMENT(CHAR_INS_GUILD_RANK, "INSERT INTO guild_rank (guildid, rid, rname, rights, BankMoneyPerDay) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_UPD_GUILD_MEMBER_RANK, "UPDATE guild_member SET guild_rank = ? WHERE guid = ?", CONNECTION_ASYNC); // 0: uint8, 1: uint32
    ...
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_RESET_TIME, "UPDATE guild_member SET BankResetTimeMoney = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC); // 0: uint32, 1: uint8
    ...
    // 0: uint32, 1: uint8
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME0, "UPDATE guild_member SET BankResetTimeTab0 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME1, "UPDATE guild_member SET BankResetTimeTab1 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME2, "UPDATE guild_member SET BankResetTimeTab2 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME3, "UPDATE guild_member SET BankResetTimeTab3 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME4, "UPDATE guild_member SET BankResetTimeTab4 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME5, "UPDATE guild_member SET BankResetTimeTab5 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME6, "UPDATE guild_member SET BankResetTimeTab6 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GUILD_RANK_BANK_TIME7, "UPDATE guild_member SET BankResetTimeTab7 = 0 WHERE guildid = ? AND guild_rank = ?", CONNECTION_ASYNC);
	...
	PREPARE_STATEMENT(CHAR_DEL_GUILD_NEWS, "DELETE FROM guild_news_log WHERE guild = ? AND id = ?", CONNECTION_ASYNC);
	...
	// Archaeology
	PREPARE_STATEMENT(CHAR_SEL_CHAR_ARCHAEOLOGY, "SELECT sites0, sites1, sites2, sites3, sites4, counts, projects, completed FROM character_archaeology WHERE guid = ?", CONNECTION_ASYNC);
	...
	PREPARE_STATEMENT(CHAR_INS_PLAYER_ARCHAEOLOGY, "INSERT INTO character_archaeology  (guid, sites0, sites1, sites2, sites3, sites4, counts, projects, completed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    // Currency
    PREPARE_STATEMENT(CHAR_SEL_PLAYER_CURRENCY, "SELECT currency, week_count, total_count, season_total, flags, weekCap FROM character_currency WHERE guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_PLAYER_CURRENCY, "UPDATE character_currency SET week_count = ?, total_count = ?, season_total = ?, flags = ?, weekCap = ? WHERE guid = ? AND currency = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_REP_PLAYER_CURRENCY, "REPLACE INTO character_currency (guid, currency, week_count, total_count, season_total, flags, weekCap) VALUES (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    // GM Tickets
    PREPARE_STATEMENT(CHAR_SEL_GM_TICKETS, "SELECT ticketId, guid, name, message, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, assignedTo, comment, completed, escalated, viewed, haveTicket FROM gm_tickets", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_REP_GM_TICKET, "REPLACE INTO gm_tickets (ticketId, guid, name, message, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, assignedTo, comment, completed, escalated, viewed, haveTicket) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_INS_GM_SUBSURVEY, "INSERT INTO gm_subsurveys (surveyId, subsurveyId, subrank, comment) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    "power4, power5, latency, speccount, activespec, specialization1, specialization2, exploredZones, equipmentCache, knownTitles, actionBars, currentpetslot, petslotused, grantableLevels) VALUES "
    "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    ...
    "equipmentCache=?,knownTitles=?,actionBars=?,currentpetslot=?,petslotused=?,grantableLevels=?,online=?, resetspecialization_cost = ?, resetspecialization_time = ? WHERE guid=?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_INS_GROUP, "INSERT INTO player_groups (guid, leaderGuid, lootMethod, looterGuid, lootThreshold, icon1, icon2, icon3, icon4, icon5, icon6, icon7, icon8, groupType, difficulty, raiddifficulty) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_UPD_GROUP_LEADER, "UPDATE player_groups SET leaderGuid = ? WHERE guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GROUP_TYPE, "UPDATE player_groups SET groupType = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_UPD_GROUP_DIFFICULTY, "UPDATE player_groups SET difficulty = ? WHERE guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_GROUP_RAID_DIFFICULTY, "UPDATE player_groups SET raiddifficulty = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHAR_CLASS_LVL_AT_LOGIN, "SELECT race, class, level, at_login, knownTitles  FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(CHAR_DEL_GROUP, "DELETE FROM player_groups WHERE guid = ?", CONNECTION_ASYNC);
    ...
    // Void Storage
    PREPARE_STATEMENT(CHAR_SEL_CHAR_VOID_STORAGE, "SELECT itemId, itemEntry, slot, creatorGuid, randomProperty, reforgeId, transmogrifyId, upgradeId, suffixFactor FROM character_void_storage WHERE playerGuid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_REP_CHAR_VOID_STORAGE_ITEM, "REPLACE INTO character_void_storage (itemId, playerGuid, itemEntry, slot, creatorGuid, randomProperty, reforgeId, transmogrifyId, upgradeId, suffixFactor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_DEL_CHAR_VOID_STORAGE_ITEMS, "DELETE FROM character_void_storage WHERE playerGuid = ?", CONNECTION_ASYNC);
	...
	// battle Pet
	PREPARE_STATEMENT(CHAR_SEL_ACCOUNT_BATTLE_PETS, "SELECT id, species, nickname, timestamp, level, xp, health, quality, breed, flags FROM account_battle_pet WHERE accountId = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_ACCOUNT_BATTLE_PET, "DELETE FROM account_battle_pet WHERE id = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_INS_ACCOUNT_BATTLE_PET, "INSERT INTO account_battle_pet (id, accountId, species, nickname, timestamp, level, xp, health, quality, breed, flags) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_UPD_ACCOUNT_BATTLE_PET, "UPDATE account_battle_pet SET nickname = ?, timestamp = ?, level = ?, xp = ?, health = ?, quality = ?, flags = ? WHERE id = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_ACCOUNT_BATTLE_PET_SLOTS, "SELECT slot1, slot2, slot3, flags FROM account_battle_pet_slots WHERE accountId = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_ACCOUNT_BATTLE_PET_SLOTS, "DELETE FROM account_battle_pet_slots WHERE accountId = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_INS_ACCOUNT_BATTLE_PET_SLOTS, "INSERT INTO account_battle_pet_slots (accountId, slot1, slot2, slot3, flags) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_UPD_ACCOUNT_BATTLE_PET_SLOTS, "UPDATE account_battle_pet_slots SET slot1 = ?, slot2 = ?, slot3 = ?, flags = ? WHERE accountId = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_BATTLEPET_DECLINED_NAME, "DELETE FROM account_battle_pet_declinedname WHERE id = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_BATTLEPET_DECLINED_NAME, "SELECT genitive, dative, accusative, instrumental, prepositional FROM account_battle_pet_declinedname WHERE id = ?", CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_INS_BATTLEPET_DECLINED_NAME, "INSERT INTO account_battle_pet_declinedname (id, genitive, dative, accusative, instrumental, prepositional) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);

	PREPARE_STATEMENT(CHAR_DEL_LFB_DATA_BY_GUID, "DELETE FROM lfb_data WHERE guid = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_LFB_DATA_BY_GUID, "SELECT state FROM lfb_data WHERE guid = ?", CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_INS_LFB_DATA_BY_GUID, "INSERT INTO lfb_data (guid, state) VALUES (?, ?)", CONNECTION_ASYNC);
	
	PREPARE_STATEMENT(CHAR_DEL_BATTLE_PET_ITEM_DATA_BY_GUID, "DELETE FROM battle_pet_item_data WHERE itemId = ?", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_SEL_BATTLE_PET_ITEM_DATA_BY_GUID, "SELECT speciesId, breedData, level FROM battle_pet_item_data WHERE itemId = ?", CONNECTION_SYNCH);
	PREPARE_STATEMENT(CHAR_INS_BATTLE_PET_ITEM_DATA_BY_GUID, "INSERT INTO battle_pet_item_data (itemId, speciesId, breedData, level) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);

    // New Loot-based Lockout system.
    PREPARE_STATEMENT(CHAR_INS_WEEKLY_BOSS_KILL, "INSERT INTO character_weekly_boss_kills (guid, entry, mapId, difficulty, looted) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_WEEKLY_BOSS_KILL, "UPDATE character_weekly_boss_kills SET looted = ? WHERE guid = ? AND entry = ? AND mapId = ? AND difficulty = ?", CONNECTION_ASYNC);
	...
	// PvPstats		
	PREPARE_STATEMENT(CHAR_INS_PVPSTATS_PLAYER, "INSERT INTO pvpstats_players (battleground_id, character_guid, winner, score_killing_blows, score_deaths, score_honorable_kills, score_bonus_honor, score_damage_done, score_healing_done, attr_1, attr_2, attr_3, attr_4, attr_5) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
	PREPARE_STATEMENT(CHAR_DEL_PVPSTATS_PLAYER, "DELETE FROM pvpstats_players WHERE battleground_id = ? AND character_guid = ?", CONNECTION_ASYNC);
