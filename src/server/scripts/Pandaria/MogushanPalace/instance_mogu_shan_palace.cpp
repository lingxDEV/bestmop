...
#define EMOTE_ACHIEVE_MISS                "MISSED ACHIEVE, NEXT TIME..."
...
public:
	...
	struct instance_mogu_shan_palace_InstanceMapScript : public InstanceScript
	{
		...
		*/
		...
		void Initialize()
		{
			SetData(DATA_ACHIEVE, NOT_STARTED);
		...
		bool SetBossState(uint32 id, EncounterState state)
		{
			...
			case DATA_GEKKAN:
				...
				if (GameObject* pGo = GetGameObjectFromStorage(instance->GetDifficulty() == DUNGEON_DIFFICULTY_HEROIC ? GO_ANCIENT_MOGU_TREASURE_HC : GO_ANCIENT_MOGU_TREASURE))
				{
					DoRespawnGameObject(pGo->GetGUID(), 7 * DAY);
					pGo->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_INTERACT_COND);
				}
				if (GameObject* pGo = GetGameObjectFromStorage(instance->GetDifficulty() == DUNGEON_DIFFICULTY_NORMAL ? GO_ANCIENT_MOGU_TREASURE : GO_ANCIENT_MOGU_TREASURE_HC))
				{
					DoRespawnGameObject(pGo->GetGUID(), DAY);
					pGo->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_INTERACT_COND);
				}
		...
		void OnCreatureCreate(Creature* pCreature)
		{
			...
			case CREATURE_BEACON:
				...
				break;
			case CREATURE_SCOUT_2:
				m_uiGlintrokGuid = pCreature->GetGUID();
				break;
			case CREATURE_QUILEN_STATUE:
				m_uiQuilenGuid = pCreature->GetGUID();
				break;
			}
		}
		...
		void OnUnitDeath(Unit* unit)
		{
			...
			if (unit->ToCreature() && unit->ToCreature()->GetEntry() == CREATURE_GEKKAN)
			{
				...
				if (GameObject* pGo = GetGameObjectFromStorage(GO_ANCIENT_MOGU_TREASURE || GO_ANCIENT_MOGU_TREASURE_HC))
					pGo->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_INTERACT_COND);

				HandleGameObject(0, true, GetGameObjectFromStorage(GO_GEKKAN_TREASURE_DOOR));
			}
			
			if (unit->ToCreature() && unit->ToCreature()->GetEntry() == CREATURE_GLINTROK_ORACLE)
			{
				SetData(DATA_ORACLE, DONE);
			}

			if (unit->ToCreature() && unit->ToCreature()->GetEntry() == CREATURE_GLINTROK_HEXXER)
			{
				SetData(DATA_HEXXER, DONE);
			}

			if (unit->ToCreature() && unit->ToCreature()->GetEntry() == CREATURE_GLINTROK_IRONHIDE)
			{
				SetData(DATA_IRONHIDE, DONE);
			}

			if (unit->ToCreature() && unit->ToCreature()->GetEntry() == CREATURE_GLINTROK_SKULKER)
			{
				SetData(DATA_SKULKER, DONE);
			}
			
			if (GetData(DATA_SKULKER) == DONE && GetData(DATA_ORACLE) == DONE && GetData(DATA_IRONHIDE) == DONE && GetData(DATA_HEXXER) == DONE)
			{
					bAchieved = true;
					isAchieve();
			}
		...
		bool isAchieve()
		{
			...
			if (bAchieved, true && GetBossState(DATA_GEKKAN) == DONE && GetData(DATA_ACHIEVE) == IN_PROGRESS)
			{
				DoCompleteAchievement(ACHIEVEMENTS_GLINTROK_N_ROLL);
				SetData(DATA_ACHIEVE, DONE);
			}
			return true;
		}
		...
		void SetData_trial_of_the_king(uint32 type, uint32 data)
		{
			...
			case TYPE_SHUFFLE_ORDER:
				...
				else
				{
					...
					if (GameObject* pGo = GetGameObjectFromStorage(GO_TRIAL_CHEST || GO_TRIAL_CHEST_HC))
			...
			case TYPE_TRIAL_CHEST:
				...
				{
					if (GameObject* pGo = GetGameObjectFromStorage(GO_TRIAL_CHEST_HC))
					{
						pGo->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_INTERACT_COND);
						DoRespawnGameObject(pGo->GetGUID(), 7 * DAY);
					}
