...
enum WarriorSpells
{
    ...
    WARRIOR_SPELL_GLYPH_OF_BURNING_ANGER        = 115946,
    WARRIOR_SPELL_GLYPH_OF_BURNING_ANGER_VISUAL = 115993,
    ...
    WARRIOR_SPELL_SWORD_AND_BOARD_PASIVE        = 46953
};
...
public:
    ...
    class spell_warr_enrage_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(constAuraEffectPtr /*aurEffect*/, int32& amount, bool& canBeRecalculated)
        {
            if (Player* _player = GetCaster()->ToPlayer())
            {
                amount = 10;
                if (_player->GetSpecializationId(_player->GetActiveSpec()) == SPEC_WARRIOR_FURY && _player->HasAura(76856))
                    amount += int32(_player->GetFloatValue(PLAYER_MASTERY) * 1.4f);
                canBeRecalculated = false;

                if (_player->HasAura(WARRIOR_SPELL_GLYPH_OF_BURNING_ANGER))
                    _player->AddAura(WARRIOR_SPELL_GLYPH_OF_BURNING_ANGER_VISUAL, _player);
            }
        }
...
public:
    ...
    class spell_warr_shouts_SpellScript : public SpellScript
    {
        ...
        void HandleOnCast()
        {
            if (Unit* Caster = GetCaster())
            {
                if (Caster->HasAura(WARRIOR_SPELL_GLYPH_OF_BLOODCURDLING_SHOUT))
                    Caster->CastSpell(Caster, WARRIOR_SPELL_BLOODCURDLING_SHOUT, true);

                if (Caster->HasAura(WARRIOR_SPELL_GLYPH_OF_MYSTIC_SHOUT))
                    Caster->CastSpell(Caster, WARRIOR_SPELL_MYSTIC_SHOUT_HOVER, true);
            }
        }
        ...
        class spell_warr_mocking_banner_AuraScript : public AuraScript
        {
            ...
            void OnTick(constAuraEffectPtr aurEff)
            {
                if (Player* player = GetTarget()->ToPlayer())
                {
                    ...
                    for (auto victim : bannerList)
                    {
                        player->CastSpell(victim, WARRIOR_SPELL_MOCKING_BANNER_TAUNT, true);
                    }
                }
            }
...
// Mocking Banner Taunt - 114198
class spell_warr_mocking_banner_taunt : public SpellScriptLoader
{
public:
        ...
        bool operator()(WorldObject* object)
        {
            ...
            if (Unit* target = object->ToUnit())
            {
                return (!caster->IsValidAttackTarget(target) || target->HasAuraType(SPELL_AURA_MOD_STEALTH) ||
                    ...
                    target->HasAura(WARRIOR_SPELL_MOCKING_BANNER_TAUNT));
            }
    ...
    class spell_warr_mocking_banner_taunt_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_warr_mocking_banner_taunt_SpellScript);

        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (Player* player = GetCaster()->ToPlayer())
            {
                targets.remove_if(MockingBannerTauntTargetCheck(player));
            }
        }

        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_warr_mocking_banner_taunt_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_warr_mocking_banner_taunt_SpellScript();
    }
};
...
class spell_warr_sword_and_board : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_sword_and_board_SpellScript : public SpellScript
        {
            ...
            bool Load()
            {
                // Since Sword and Board  is learn on level 50, 
                // this script sould not work for any player until then.
                if (!GetCaster()->HasAura(WARRIOR_SPELL_SWORD_AND_BOARD_PASIVE))
                    return false;
                return true;
            }
	...
	class spell_warr_shockwave_SpellScript : public SpellScript
	{
		...
		void HandleOnHit()
		{
			if (Player* _player = GetCaster()->ToPlayer())
			{
				if (Unit* target = GetHitUnit())
				{
					...
					// Storing The Spell Targets for later Use
					HitTargets++;
				}
			}
		}
		...
		void HandleAfterCast()
		{
			if (Player* _player = GetCaster()->ToPlayer())
			{
				if (HitTargets >= 3)
				{
					_player->ReduceSpellCooldown(46968, 20000);
					HitTargets = 0;
				}
			}
		}
                    ...
                    if (Unit* target = GetHitUnit())
                    {
                        if (target->GetTypeId() != TYPEID_PLAYER)
                        {
                            if (caster->ToPlayer() && !caster->ToPlayer()->HasSpellCooldown(WARRIOR_SPELL_IMPENDING_VICTORY))
                            {
                                ...
                                if (caster->HasAura(58357)) // Glyph of Gab Order
...
class spell_warr_glyph_of_hamstring : public SpellScriptLoader
{
    public:
        ...
        class spell_warr_glyph_of_hamstring_SpellScript : public SpellScript
        {
            ...
            {
                if (Unit* caster = GetCaster())
                    if (caster->HasAura(58385) && !caster->HasAura(115945))
...
/*######
## spell_warr_unbridled_wrath  - 143268
######*/
...
public:
	...
	class spell_warr_unbridled_wrath_AuraScript : public AuraScript
	{
		...
		{
			if (Unit* caster = GetCaster())
				return caster->HasAuraWithMechanic((1 << MECHANIC_ROOT) | (1 << MECHANIC_STUN) | (1 << MECHANIC_SAPPED));
			return false;
		}

		void Register()
		{
			DoCheckProc += AuraCheckProcFn(spell_warr_unbridled_wrath_AuraScript::OnProc);
		}
	};
...
/*######
## spell_warr_shield_slam_energize - 23922
######*/

class spell_warr_shield_slam_energize : public SpellScriptLoader
{
public:
	spell_warr_shield_slam_energize() : SpellScriptLoader("spell_warr_shield_slam_energize") { }

	class spell_warr_shield_slam_energize_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_warr_shield_slam_energize_SpellScript);

		void HandleEffect(SpellEffIndex effIndex)
		{
			if (Unit* caster = GetCaster())
				if (!caster->HasAura(SPELL_WARRIOR_DEFENSIVE_STANCE))
					PreventHitEffect(effIndex);
		}

		void Register()
		{
			OnEffectHitTarget += SpellEffectFn(spell_warr_shield_slam_energize_SpellScript::HandleEffect, EFFECT_2, SPELL_EFFECT_ENERGIZE);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_warr_shield_slam_energize_SpellScript;
	}
};

/*######
## spell_warr_revenge_energize - 6572
######*/

class spell_warr_revenge_energize : public SpellScriptLoader
{
public:
	spell_warr_revenge_energize() : SpellScriptLoader("spell_warr_revenge_energize") { }

	class spell_warr_revenge_energize_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_warr_revenge_energize_SpellScript);
		...
		void Register()
		{
			OnEffectHitTarget += SpellEffectFn(spell_warr_revenge_energize_SpellScript::HandleEffect, EFFECT_1, SPELL_EFFECT_ENERGIZE);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_warr_revenge_energize_SpellScript;
	}
};
...
void AddSC_warrior_spell_scripts()
{
    ...
    new spell_warr_mocking_banner_taunt();
	...
	new spell_warr_shield_slam_energize();
	new spell_warr_revenge_energize();
}
