...
class npc_rebel_cannon_phase_4  : public CreatureScript
{
    ...
    private:
        ...
        struct creature_script_impl  : public VehicleAI
        {
			...
			void UpdateHealth(){
				if (Battleground* battle = me->GetBattleground()){
					Player* victim = me->SelectNearestPlayer(1000.0f);
					if (battle->GetTypeID(true) == BATTLEGROUND_SA && victim && victim->getLevel() < 86 && victim->getLevel() > 79){
						me->SetMaxHealth(victim->GetMaxHealth() * 3);
...
class npc_catapult_battlegrounSA : public CreatureScript
{
public:
	npc_catapult_battlegrounSA() : CreatureScript("npc_catapult_battlegrounSA") { }

private:

	CreatureAI* GetAI(Creature* creature) const
	{
		return new creature_script_catapult_battlegroundSA(creature);
	}

	struct creature_script_catapult_battlegroundSA : public VehicleAI
	{
		InstanceScript* pInstance;

		creature_script_catapult_battlegroundSA(Creature* creature) : VehicleAI(creature) {
			pInstance = creature->GetInstanceScript();
		}
...
enum eDarkRangerThyala
{
	QUEST_LEADER_OF_THE_PACKLEADER_OF_THE_PACK = 14386,
...
enum eDarkRangerThyalaEvents
{
	EVENT_ARK_RANGER_THYALA_CAST_SPELL_KNOCKBACK = 1,
	EVENT_ARK_RANGER_THYALA_CAST_SPELL_SHOOT = 2,
};
...
public:
	...
	struct npc_dark_ranger_thyalaAI : public ScriptedAI
	{
		...
		{
			events.ScheduleEvent(EVENT_ARK_RANGER_THYALA_CAST_SPELL_KNOCKBACK, irand(6000, 8000));
			events.ScheduleEvent(EVENT_ARK_RANGER_THYALA_CAST_SPELL_SHOOT, irand(9000, 12000));
		}
		...
		void JustDied(Unit* killer)
		{
			...
			for (auto player : playerList)
			{
				...
				{
					if (player->GetQuestStatus(QUEST_LEADER_OF_THE_PACKLEADER_OF_THE_PACK) == QUEST_STATUS_INCOMPLETE)
					{
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				switch (eventId)
				{
				case EVENT_ARK_RANGER_THYALA_CAST_SPELL_KNOCKBACK:
					if (me->getVictim())
						me->CastSpell(me->getVictim(), SPELL_KNOCKBACK, true);
					events.ScheduleEvent(EVENT_ARK_RANGER_THYALA_CAST_SPELL_KNOCKBACK, irand(6000, 8000));
					break;
				case EVENT_ARK_RANGER_THYALA_CAST_SPELL_SHOOT:
					if (me->getVictim())
...
void AddSC_gilneas()
{
	new npc_catapult_battlegrounSA();
