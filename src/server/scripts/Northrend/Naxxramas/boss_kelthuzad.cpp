    ...
    struct boss_kelthuzadAI : public BossAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			if (instance)
			{
				...
				if (PlayerList.getSize() < 21 && map->GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
					instance->DoCompleteAchievement(579);

				// Checking For Feat of Strength (Conqueror of Naxxramas)
				QueryResult result = CharacterDatabase.PQuery("SELECT guid FROM character_achievement WHERE achievement = 1402 LIMIT 1");
				// No Achievements were found Realm-First
				if (!result)
				{
					instance->DoCompleteAchievement(1402);
				}

				// Achievement (Never is Enough id 2185)
				if (instance->GetData(DATA_ABOMINATION_KILLED) >= 18 && map->GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
					instance->DoCompleteAchievement(2185);

				// Achievement The Undying
				if (!instance->GetData(DATA_PLAYER_KILLED) == 1 && map->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
					instance->DoCompleteAchievement(142);
				// Achievement The Inmortal
				if (!instance->GetData(DATA_PLAYER_KILLED) == 1 && map->GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
					instance->DoCompleteAchievement(141);

				// Kel'thuzad Defeat Achievements
				switch (map->GetDifficulty())
				{
					case RAID_DIFFICULTY_10MAN_NORMAL:
						instance->DoCompleteAchievement(574);
						break;
					case RAID_DIFFICULTY_25MAN_NORMAL:
						instance->DoCompleteAchievement(575);
						break;
					default: return;
				}
			}
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            Phase = 1;
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_NOT_SELECTABLE);
            me->SetFloatValue(UNIT_FIELD_BOUNDING_RADIUS, 4.0f);
            me->SetFloatValue(UNIT_FIELD_COMBAT_REACH, 4.0f);
                ...
                else if (nGuardiansOfIcecrownCount < RAID_MODE(2, 4))
                {
                    if (uiGuardiansOfIcecrownTimer <= diff)
                    {
                        ...
                        if (Creature* pGuardian = DoSummon(NPC_ICECROWN, Pos[RAND(2, 5, 8, 11)]))
                            pGuardian->SetFloatValue(UNIT_FIELD_COMBAT_REACH, 2.0f);
