...
class npc_warmaster_blackhorn_goriona: public CreatureScript
{
    public:
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_TWILIGHT_ONSLAUGHT:
                        {
                            ...
                            if (!creatures.empty())
                                if (Unit* pTarget = WoWSource::Containers::SelectRandomContainerElement(creatures))
                                {
        ...
        private:
            ...
            struct TwilightFlamesSelector : public zonaxtrema::unary_function<Unit*, bool>
            {
                public:
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DEGENERATION:                            
                            ...
                            {
                                if (AuraPtr aur = pTarget->GetAura(SPELL_DEGENERATION))
                        ...
                        case EVENT_BRUTAL_STRIKE:                            
                            if (Unit* pTarget = me->getVictim())
                            {
                                if (AuraPtr aur = pTarget->GetAura(SPELL_BRUTAL_STRIKE))
                                    aur->ModStackAmount(1);
                                else
    ...
    public:
        ...
        class spell_warmaster_blackhorn_shockwave_aoe_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                {
                    targets.clear();
                    targets.push_back(WoWSource::Containers::SelectRandomContainerElement(new_targets));
                }
                else
                    WoWSource::Containers::RandomResizeList(targets, 1);
            }
