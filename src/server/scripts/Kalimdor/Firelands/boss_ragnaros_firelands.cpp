			...
			void JustDied(Unit* /*killer*/)
			{
				...
				if (!IsHeroic())
				{
					...
					if (map->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
						me->SummonGameObject(GO_CACHE_OF_THE_FIRELORD_10, 1016.043f, -57.436f, 55.333f, 3.151f, 0, 0, 0, 0, 30000);
					else if (map->GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL)
						me->SummonGameObject(GO_CACHE_OF_THE_FIRELORD_25, 1016.043f, -57.436f, 55.333f, 3.151f, 0, 0, 0, 0, 30000);
					Talk(SAY_DEATH_1);
					return;
				}
				...
				// Despawning Boss Door
				if (GameObject* door = me->FindNearestGameObject(GO_SULFURON_KEEP, 200.0f))
                    ...
                    else
                    {
						...
						if (!playerList.empty())
						{
							for (std::list<Player*>::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
							{
								...
								{
									me->CastSpell(pPlayer, SPELL_SULFURAS_SMASH_DMG, true);
								}
							}
						}
					}
    ...
    public:
        ...
        class spell_ragnaros_firelands_blazing_speed_AuraScript : public AuraScript
        {
            ...
            void HandlePeriodicTick(constAuraEffectPtr /*aurEff*/)
            {
                ...
                {
                    case RAID_DIFFICULTY_10MAN_NORMAL: spell_id = SPELL_BLAZING_SPEED; break;
                    case RAID_DIFFICULTY_25MAN_NORMAL: spell_id = SPELL_BLAZING_SPEED_25; break;
                    case RAID_DIFFICULTY_10MAN_HEROIC: spell_id = SPELL_BLAZING_SPEED_10H; break;
                    case RAID_DIFFICULTY_25MAN_HEROIC: spell_id = SPELL_BLAZING_SPEED_25H; break;

					default:                           spell_id = SPELL_BLAZING_SPEED; break;
                }
